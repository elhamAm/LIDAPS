...
scale=56
model = dict(
    type='MaskRCNNPanopticDiffUnfrozenT0ScaleRoiWhereScale',
    ...
    rpn_head=dict(
        ...
        loss_cls=dict(type='CrossEntropyLoss', use_sigmoid=True, loss_weight=0),#maskrcnn_losses_weights),
        loss_bbox=dict(type='L1Loss', loss_weight=0)),#maskrcnn_losses_weights)),
    roi_head=dict(
    ...
    train_cfg=dict(
        folder='continueRPN'+str(scale)+'_920',
        rpn=dict(
            assigner=dict(
                type='MaxIoUAssigner',
