...
model = dict(
    type='MaskRCNNPanopticDiffFrozenT0ScaleRoiBothClipGradient',
    ...
    train_cfg=dict(
        folder='diff_roi_frozen'+str(scale)+'_bothWeighted_clipgradient_withSemantic',
        rpn=dict(
            assigner=dict(
                type='MaxIoUAssigner',
    ...
    test_cfg=dict(
        ...
        rpn=dict(
                    nms_pre=1000,
                    max_per_img=1000,
                    nms=dict(type='nms', iou_threshold=0.7),#changing the nms
                    min_bbox_size=0
                ),
        rcnn=dict(
                    score_thr=0.05,
                    nms=dict(type='nms', iou_threshold=0.5),#changing the nms
                    max_per_img=100,
                    mask_thr_binary=0.5
                )
                )
