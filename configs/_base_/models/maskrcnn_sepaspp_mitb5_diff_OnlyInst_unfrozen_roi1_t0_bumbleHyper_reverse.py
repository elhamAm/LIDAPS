...
model = dict(
    type='MaskRCNNPanopticDiffUnfrozenT0Reverse',
    pretrained='pretrained/mit_b5.pth',
    backbone=dict(type='mit_b5', style='pytorch'),
    use_neck_feat_for_decode_head=False,
    decode_head=dict(
        type='DAFormerHead',
        in_channels=[1280, 1280, 640, 320],# [64, 128, 320, 512],##,#[320, 790, 1430, 1280]
        in_index=[0, 1, 2, 3],
        channels=256,
        dropout_ratio=0.1,
        num_classes=19,
    ...
    neck=dict(
        type='FPN',
        in_channels=[1280, 1280, 640, 320],#[320, 660, 1300, 1280],#,#[1280, 1300, 660, 320],#,#[1280,1300, 660, 320],#[320, 660, 1300, 1280],#[64, 128, 320, 512],#,#,[,#
        out_channels=256,
        num_outs=5),
    rpn_head=dict(
        type='RPNHead',
    ...
    roi_head=dict(
        ...
        bbox_head=dict(
            ...
            loss_cls=dict(type='CrossEntropyLoss', use_sigmoid=False, loss_weight=0),#maskrcnn_losses_weights),#maskrcnn_losses_weights),
            loss_bbox=dict(type='L1Loss', loss_weight=0)),#maskrcnn_losses_weights)),#maskrcnn_losses_weights)),
    ...
    train_cfg=dict(
        rpn=dict(
            assigner=dict(
                type='MaxIoUAssigner',
                pos_iou_thr=0.53,#0.7,#0.9, #0.7,
                neg_iou_thr=0.38,#0.3,#0.1,
                min_pos_iou=0.4,#0.3,#0.1,
