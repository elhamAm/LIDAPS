...
def computeAverages(aps,args, logger):
    ...
    avgDict["allAp_6"]    = np.nanmean(np.take(aps,[0,1,2,4,6,7],axis=1)[ dInf,:,:  ])
    avgDict["allAp50%_6"]   = np.nanmean(np.take(aps,[0,1,2,4,6,7],axis=1)[ dInf,:,o50])
    ...
    avgDict["classes"]  = {}
    #[lI for (lI,labelName) in enumerate(args.instLabels) ] [0, 1, 2, 3, 4, 5, 6, 7]
    #[labelName for (lI,labelName) in enumerate(args.instLabels) ] ['person', 'rider', 'car', 'truck', 'bus', 'train', 'motorcycle', 'bicycle']
    for (lI,labelName) in enumerate(args.instLabels):
...
def printResults(avgDict, args, logger):
    ...
    allApAvg_6  = avgDict["allAp_6"]
    allAp50o_6  = avgDict["allAp50%_6"]
    if args.distanceAvailable:
    ...
    line += getColorEntry(allApAvg_6 , args) + sep + "{:>15.3f}".format(allApAvg_6)  + sep
    line += getColorEntry(allAp50o_6 , args) + sep + "{:>15.3f}".format(allAp50o_6)  + sep
    if args.distanceAvailable:
...
def evaluateImgLists(
    ...
    if True:
        writeDict2JSON(matches,"PATH_TO_FILL/matches.json")
