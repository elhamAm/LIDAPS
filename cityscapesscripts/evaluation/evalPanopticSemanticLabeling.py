...
class PQStat():
    ...
    def pq_average(self, categories, isthing):
        pq, sq, rq, n = 0, 0, 0, 0
        pq_16, sq_16, rq_16, n_16 = 0,0,0,0
        per_class_results = {}
        for label, label_info in categories.items():
            ...
            sq += sq_class
            rq += rq_class
            if(label_info['name'] != 'terrain' and label_info['name'] != 'train' and label_info['name'] != 'truck'):
                pq_16 += pq_class
                sq_16 += sq_class
                rq_16 += rq_class
                n_16 += 1
        ...
        return {'pq': pq / n, 'sq': sq / n, 'rq': rq / n, 'n': n, 'pq_16': pq_16 / n_16, 'sq_16': sq_16 / n_16, 'rq_16': rq_16 / n_16, 'n_16': n_16}, per_class_results
...
def pq_compute_single_core(proc_id, annotation_set, gt_folder, pred_folder, categories,
    ...
    for gt_ann, pred_ann in annotation_set:
        ...
        if 'mapillary' in dataset_name:
            ...
            print('where the image should be opened: ', os.path.join(gt_folder, sub_folder_name, gt_ann['file_name']))
...
def print_results(results, categories, logger):
    ...
    for name in metrics:
        logger.info("{:14s}| {:5.1f}  {:5.1f}  {:5.1f} {:5d} {:5.1f}  {:5.1f}  {:5.1f} {:5d}".format(
            name,
            100 * results[name]['pq'],
            100 * results[name]['sq'],
            100 * results[name]['rq'],
            results[name]['n'],
            100 * results[name]['pq_16'],
            100 * results[name]['sq_16'],
            100 * results[name]['rq_16'],
            results[name]['n_16']
        ))
