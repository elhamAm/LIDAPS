...
def register_hier_output(model):
    ...
    def forward(x, timesteps=None, context=None, y=None,**kwargs):
        ...
        outs_in=[]
        #this is encoding
        for i_in, module in enumerate(self.input_blocks):
            # import pdb; pdb.set_trace()
            h = module(h, emb, context)
            hs.append(h)
            #print('input h.shape: ', h.shape)
            #breakpoint()
            if i_in in [2, 5, 8]:
                outs_in.append(h)
                ...
                #breakpoint()
        outs_in.append(h)
        #save_image((h[0,:3,:,:]+1.)/2., 'PATH_TO_FILL/try'+str(i_in)+'.png')
        #breakpoint()
...
class UNetWrapper(nn.Module):
    ...
    def forward(self, isEval, *args, **kwargs):
        ...
        #                 #breakpoint()
        #                 one=cross_maps = item.reshape(2, res, res, item.shape[-1])[0,:,:,0]
        #                 from torchvision.utils import save_image
        #                 three=torch.cat([one.unsqueeze(0),one.unsqueeze(0),one.unsqueeze(0)],dim=0)
        #                 #breakpoint()
        #                 #save_image(three, 'img_'+str(self.count)+'/'+'attention_'+'_'+str(t)+'.png')
        #                 #print('attention: ', str(self.count))
        #                 #save_image(three, 'attention_'+str(self.count)+'_'+str(t)+'.png')
        #                 t+=1
        ...
        #     #tokenizer=FrozenCLIPEmbedder(max_length=20)
        ...
        #breakpoint()
        #len(out_list) 4
        #0 torch.Size([2, 1280, 8, 8]) 
        #1 torch.Size([2, 1280, 16, 16])
        #2 torch.Size([2, 640, 32, 32])
        #3 torch.Size([2, 320, 64, 64])
        #show_cross_attention(self.attention_store, res=16, from_where=["up", "down"])
        #breakpoint()
        if self.use_attn:
            #breakpoint()
...
class FrozenClipImageEmbedder(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        import clip
        self.model, self.preprocesss = clip.load(name=model, device=device, jit=jit)
    ...
    def forward(self, x):
        ...
        breakpoint()
        return self.model.encode_image(preprocesss(x))
