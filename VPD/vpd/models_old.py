...
def register_attention_control(model, controller):
    ...
    def ca_forward(self, place_in_unet):
        ...
        def forward(x, context=None, mask=None):
            ...
            #def reshape_heads_to_batch_dim(t, tensor):
...
class AttentionStore(AttentionControl):
    ...
    def forward(self, attn, is_cross: bool, place_in_unet: str):
        ...
        if attn.shape[1] <= (self.max_size) ** 2:  # avoid memory overhead
            #print('keeping this attn: ', attn.shape)
    ...
    def between_steps(self):
        ...
        #print('between steps: ', self.step_store)

    def get_average_attention(self):
        #for key in self.step_store:
        #    for item in self.step_store[key]:
        #        print('what is the shape at this point: ', item.shape, 'key: ', key)
    ...
    def __init__(self, base_size=64, max_size=None, isEval=False):
        ...
        #print('max size in init: ', self.max_size)
...
def register_hier_output(model):
    ...
    def forward(x, timesteps=None, context=None, y=None,**kwargs):
        ...
        for i_out, module in enumerate(self.output_blocks):
            ...
            #print('i_out, h.shape before module: ', i_out, h.shape)
            #breakpoint()
            h = module(h, emb, context)
            #print('i_out, h.shape after module: ', i_out, h.shape)
            if i_out in [1, 4, 7]:
                out_list.append(h)
        h = h.type(x.dtype)
        ...
        out_list.append(h)
        #breakpoint()
        #print('length of out_list: ', len(out_list))
        #print(out_list[0].shape, out_list[1].shape, out_list[2].shape, out_list[3].shape)
        return out_list
        
    self.forward = forward

class UNetWrapper(nn.Module):
    def __init__(self, unet, use_attn=True, base_size=1024, max_attn_size=None, attn_selector='up_cross+down_cross') -> None:
        super().__init__()
        #breakpoint()
        self.unet = unet
        self.attention_store = AttentionStore(base_size=base_size // 8, max_size=max_attn_size, isEval=False)
        ...
        #breakpoint()
    def forward(self, isEval,*args, **kwargs):
        #breakpoint()
        #print('#################################################')
        self.attention_store.isEval=isEval
        if self.use_attn:
        ...
        if self.use_attn:
            ...
            #print('####################### attn16: ', attn16.shape)
            
            # out_list[1] torch.Size([2, 1280, 8, 8])
            # out_list[1] torch.Size([2, 1280, 16, 16])
            # out_list[2]  torch.Size([2, 640, 32, 32])
            # out_list[3] torch.Size([2, 320, 64, 64])
            
            out_list[1] = torch.cat([out_list[1], attn16], dim=1)
            #out_list[2] = torch.cat([out_list[2], attn32], dim=1)
            if attn32 is not None:
                #print('####################### attn32: ', attn32.shape)
                out_list[2] = torch.cat([out_list[2], attn32], dim=1)
            if attn64 is not None:
                #print('####################### attn64: ', attn32.shape)
                out_list[3] = torch.cat([out_list[3], attn64], dim=1)
                #print('####################### attn64: ', attn64.shape)
        return out_list[::-1]

    def process_attn(self, avg_attn):
        #breakpoint()
        #if(avg_attn['up_cross'][0].shape[1] < 256):
        #    self.size16=8
        #    self.size32=16
        #    self.size64=32
        self.size16=16
        ...
        for k in self.attn_selector:
            for up_attn in avg_attn[k]:
                size = 2**int(math.log(int(math.sqrt(up_attn.shape[1])),2))
                # size 32 16
                #print('###size: ', size)
                # k up_cross/down_cross
                #print('####k: ', k)
                #2, 1024, 20
                #print('#####up_attn: ', up_attn.shape)
                #print('rearranged: ', rearrange(up_attn, 'b (h w) c -> b c h w', h=size).shape)
                attns[size].append(rearrange(up_attn, 'b (h w) c -> b c h w', h=size))
        #print('torch.stack(attns[self.size16]): ', torch.stack(attns[self.size16]).shape)
        #torch.stack(attns[self.size16]):  torch.Size([5, 2, 20, 16, 16])
        attn16 = torch.stack(attns[self.size16]).mean(0)
        
        if len(attns[self.size32]) > 0:
