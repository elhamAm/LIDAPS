...
def get_cityscapes_instance_format_for_maskrcnn(
    ...
    for c in range(num_classes):
        ...
        if boxes_c.shape[0] > 0:
            masks_c = masks[c]
            #breakpoint()
            if(masks_feat is not None):
                masks_feat_c = masks_feat[c]
            assert boxes_c.shape[0] == len(masks_c), 'there must be same number of masks and boxes for a class!'
            for m in range(len(masks_c)):
                ...
                if mask_score >= mask_score_th:
                    ...
                    if(c==2):
                        ...
                        color+=20
                        if(masks_feat is not None):
                            ins_seg_mask_cars_feat.append(masks_feat_c[m])
    ...
    #breakpoint()
    return instances, ins_seg, None, ins_seg_cars, ins_cars, ins_seg_mask_cars_feat, boxes_cars
