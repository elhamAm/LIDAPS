...
def update_configs_with_eval_paths(cfg):
    ...
    #breakpoint()
    checkpoint_file_path = cfg.checkpoint_path #os.path.join(cfg.checkpoint_path, 'iter_40000.pth')#'latest.pth')
...
def main(args):
    ...
    #model = build_segmentor(cfg.uda, test_cfg=cfg.get('test_cfg'))
    from mmseg.models.builder import build_train_model
    model = build_train_model(cfg, train_cfg=cfg.get('train_cfg'), test_cfg=cfg.get('test_cfg')).eval()
    #breakpoint()
    ...
    checkpoint = load_checkpoint( model, pathDict['checkpoint_file_path'], map_location='cpu')
    #breakpoint()
