...
def main(args):
    ...
    if 'DenseCLIP' in cfg.model.type:
        ...
        cfg.model.class_names = list(classes)
    #
    #breakpoint()
    ...
    if cfg.checkpoint_path:
        ...
        checkpoint_file_path = checkpoint_path #os.path.join(checkpoint_path, 'iter_40000.pth')#'iter_7000.pth') #'iter_5400.pth')
        #breakpoint()
    ...
    #cfg.evaluate_from_saved_numpy_predictions=True
    #breakpoint()
    if not distributed:
        print('in not distributed case')
        model = build_dp(model, cfg.device, device_ids=cfg.gpu_ids)
        #breakpoint()
        if cfg.generate_only_visuals_without_eval:
            print('in this visual debug')
        ...
        elif cfg.evaluate_from_saved_png_predictions or cfg.evaluation.evaluate_from_saved_numpy_predictions:
            ...
            #entered here when notting is wriiten
    ...
    if cfg.generate_only_visuals_without_eval:
        pass
    #here ented when nothing is written
        ...
        if json_file is not None and rank == 0:
            mmcv.dump(metric_dict, json_file)
    elif cfg.evaluation.evaluate_from_saved_numpy_predictions and not cfg.dump_predictions_to_disk and not outputs:
        cfg['evaluation']['panop_eval_temp_folder'] = cfg['panop_eval_temp_folder_previous']
        cfg['evaluation']['evaluate_from_saved_numpy_predictions'] = cfg.evaluation.evaluate_from_saved_numpy_predictions
        #breakpoint()
        eval_kwargs = cfg.get('evaluation', {}).copy()
        for key in ['interval', 'tmpdir', 'start', 'gpu_collect', 'save_best', 'rule', 'dynamic_intervals']:
            eval_kwargs.pop(key, None)
