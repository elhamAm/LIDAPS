...
class SimpleFeaturePyramid(BaseModule):
    ...
    def forward(self, x):
        ...
        #breakpoint()
        features = x[0]#bottom_up_features[self.in_feature]
        #breakpoint()
        results = []
...
#https://github.com/facebookresearch/detectron2/blob/main/detectron2/modeling/backbone/vit.py#L363
@NECKS.register_module()
class DecSimpleFeaturePyramid2(BaseModule):
    r"""Feature Pyramid Network.
    ...
    def __init__(
        ...
        Args:
            ...
            square_pad (int): If > 0, require input images to be padded to specific square size.
        """
        super(DecSimpleFeaturePyramid2, self).__init__()
        #assert isinstance(net, Backbone)
        #super(SimpleFeaturePyramid, self).__init__(init_cfg)
        #breakpoint()
        self.scale_factors = scale_factors
    ...
    def forward(self, x):
        ...
        #bottom_up_features = self.net(x)
        #breakpoint()
        features = x[2]#bottom_up_features[self.in_feature]
        #breakpoint()
        results = []
        #breakpoint()
...
#https://github.com/facebookresearch/detectron2/blob/main/detectron2/modeling/backbone/vit.py#L363
@NECKS.register_module()
class DecSimpleFeaturePyramidAll256(BaseModule):
    r"""Feature Pyramid Network.
    ...
    def __init__(
        self,
        out_channels,
        scale_factors=(8.0, 4.0, 2.0, 1.0, 0.5),#(4.0, 2.0, 1.0, 0.5),
        top_block=None,
        norm="LN",
        square_pad=0,
    ):
        ...
        Args:
            ...
            square_pad (int): If > 0, require input images to be padded to specific square size.
        """
        super(DecSimpleFeaturePyramidAll256, self).__init__()
        #assert isinstance(net, Backbone)
        #super(SimpleFeaturePyramid, self).__init__(init_cfg)
        #breakpoint()
        self.scale_factors = scale_factors
        ...
        #breakpoint()
        #strides = [int(input_shapes[in_feature].stride / scale) for scale in scale_factors]
        strides = [4,2,1,0.5,0.25]#[int(input_shapes[in_feature].stride / scale) for scale in scale_factors]
        #_assert_strides_are_log2_contiguous(strides)

        out_channelss = [256,256,256,256,256]#[768,768,768,768]#input_shapes[in_feature].channels
        self.stages = []
        #dims=[320,660,1300,1280]
        #breakpoint()
    ...
    def forward(self, x):
        ...
        #breakpoint()
        #breakpoint()
        features = Fun.interpolate(features, [features.shape[2]*2, features.shape[3]*2], mode='bilinear', align_corners=True)
        results = []
        #breakpoint()
