...
from mmdet.models.utils import preprocess_panoptic_gt_for_cityscapes
# from mmdet.models.utils import preprocess_panoptic_gt # original
...
class MaskFormerHead(AnchorFreeHead):
    ...
    def __init__(self,
                 ...
                 loss_cls=dict(type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1.0, class_weight=[1.0] * 133 + [0.1]),
                 loss_mask=dict(type='FocalLoss', use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=20.0),
                 loss_dice=dict(type='DiceLoss', use_sigmoid=True, activate=True, naive_dice=True, loss_weight=1.0),
                 train_cfg=None,
                 test_cfg=None,
                 init_cfg=None,
                 **kwargs):
    ...
    def preprocess_gt(self, gt_labels_list, gt_masks_list, gt_semantic_segs,
        ...
        """
        # num_things_list = [self.num_things_classes] * len(gt_labels_list) # original
        # num_stuff_list = [self.num_stuff_classes] * len(gt_labels_list)  # original
        if gt_semantic_segs is None:
            gt_semantic_segs = [None] * len(gt_labels_list)
        cityscapes_thing_list = [self.cityscapes_thing_list] * len(gt_labels_list) # added by Suman
        num_classes_list = [self.num_classes] * len(gt_labels_list)  # added by Suman
        targets = multi_apply(preprocess_panoptic_gt_for_cityscapes, gt_labels_list, gt_masks_list,
                              gt_semantic_segs, img_metas, cityscapes_thing_list, num_classes_list)  # added by Suman
    ...
    def get_targets(self, cls_scores_list, mask_preds_list, gt_labels_list, gt_masks_list, img_metas):
        ...
        (labels_list, label_weights_list, mask_targets_list, mask_weights_list, pos_inds_list, neg_inds_list) = \
            multi_apply(self._get_target_single, cls_scores_list, mask_preds_list, gt_labels_list, gt_masks_list, img_metas)
    ...
    def _get_target_single(self, cls_score, mask_pred, gt_labels, gt_masks,  img_metas):
        ...
        Args:
            cls_score (Tensor): Mask score logits from a single decoder layer for one image. Shape (num_queries, cls_out_channels).
            mask_pred (Tensor): Mask logits for a single decoder layer for one image. Shape (num_queries, h, w).
            gt_labels (Tensor): Ground truth class indices for one image with shape (n, ). n is the sum of number of stuff type and number of instance in a image.
            gt_masks (Tensor): Ground truth mask for each image, each with shape (n, h, w).
    ...
    def forward(self, feats, img_metas):
        ...
        mask_features, memory = self.pixel_decoder(feats, img_metas) # calling TransformerEncoderPixelDecoder.forward()
        ...
        query_embed = self.query_embed.weight # self.query_embed = nn.Embedding(self.num_queries, out_channels)
    ...
    def forward_train(self, feats, img_metas, gt_bboxes, gt_labels, gt_masks, gt_semantic_seg, gt_bboxes_ignore=None, update_output_debug=False,  ):
        ...
        # loss
        #breakpoint()
        maskformer_losses = self.loss(all_cls_scores, all_mask_preds, gt_labels, gt_masks, img_metas)
        losses = dict()

        # all_cls_scores and all_mask_preds are required by the maskformer forward()
        # to generate prediction for visual debug
        if update_output_debug:
            losses['all_cls_scores'] = all_cls_scores
            losses['all_mask_preds'] = all_mask_preds
