...
from mmdet.models.detectors.clipOrig import AttentionPool2dDense
from mmdet.models.detectors.clipDense import ContextDecoder
from mmdet.models.detectors.clipDense import CLIPTextContextEncoder
import clip

#breakpoint()
@DETECTORS.register_module()
class MaskRCNNPanopticContextTextRefineContext(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 auxiliary_head=None):
        super(MaskRCNNPanopticContextTextRefineContext, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                roi_head=roi_head,
        ...
        self.text_encoder = CLIPTextContextEncoder(context_length=13,embed_dim=512,transformer_width=512,transformer_heads=8,transformer_layers=12)#.eval()
        self.text_encoder.init_weights()
        context_length = self.text_encoder.context_length - self.context_length
        token_embed_dim=512
        #breakpoint()

        self.contexts = torch.nn.Parameter(torch.randn(1, context_length, token_embed_dim))
        torch.nn.init.trunc_normal_(self.contexts)

        class_names=['person', 'rider', 'car', 'bus', 'motorcycle', 'bicycle']
        self.texts = torch.cat([tokenize(c, context_length=self.context_length) for c in class_names])
        self.num_classes = len(self.texts)
        
        #if(denseClip):
        #    self.clip=CLIPVisionTransformer(width=768,patch_size=16,layers=12,out_indices=[3, 5, 7, 11], output_dim=512,input_resolution=512,get_embeddings=False) 
        #    self.clip.init_weights('PATH_TO_FILL/pretrained/ViT-B-16.pt')
        #else:
        #    model=torch.jit.load('PATH_TO_FILL/pretrained/ViT-B-16.pt', map_location="cpu")#.eval()
        #    state_dict=model.state_dict()
        #    self.clip=build_model(state_dict).float().to('cuda')#.visual
        #breakpoint()
        self.attnPool=AttentionPool2dDense(spacial_dim=16, embed_dim=512, num_heads=8, output_dim=512)
        self.context_decoder=ContextDecoder(context_length=16, transformer_width=256, transformer_heads=4,transformer_layers=3,visual_dim=512,dropout=0.1,outdim=512)
        self.gamma = torch.nn.Parameter((torch.ones(512) * 1e-4).to('cuda'))
        #breakpoint()
    ...
    def extract_feat(self, img):
        ...
        #texts=['person', 'rider', 'car', 'bus', 'motorcycle', 'bicycle']
        # (B, K, C)
        #breakpoint()
        #breakpoint()
        B=img.shape[0]

        text_embeddings = self.text_encoder(self.texts.to('cuda'), self.contexts).to('cuda').expand(B, -1, -1)
        ...
        # (B, 1, C)
        global_feat, visual_embeddings=self.attnPool(x_backbone[-1])
        B, C, H, W = visual_embeddings.shape
        #visual_context shape torch.Size([2, 257, 512])
        ...
        #breakpoint()
        #dim=1?
        x_backbone[-1] = torch.cat([x_backbone[-1], score_map], dim=1)
