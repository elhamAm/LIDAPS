...
class MaskRCNNPanopticDevilInstTextCrossEntropy(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 auxiliary_head=None):
        super(MaskRCNNPanopticDevilInstTextCrossEntropy, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                roi_head=roi_head,
        ...
        self.clipVision, self.preprocess = clip.load("ViT-B/32", device=device)   
        ...
        if gt_bboxes:
            ...
            if not set_loss_to_zero:
                ...
                for j in range(2):
                    for i in range(gt_bboxes[j].shape[0]):
                        ...
                        if(x_tl+1<x_br and y_tl+1<y_br):
                            ...
                            if(sample.shape[1] > 25 or sample.shape[2] > 25):
                                #breakpoint()
                                gt_feat_clip.append(self.class_embeddings[gt_labels[j][i]])
                                #samples.append(trans_sample.squeeze(0))
                                bbox_feat_gt_.append(bbox_feat_gt[i])
                                #breakpoint()
                                gt_labels_select.append(gt_labels[j][i])
                ...
                if(len(bbox_feat_gt_)>0):
                    ...
                    gt_label_selected=torch.stack(gt_labels_select)
                    #breakpoint()
                    #with torch.no_grad():
                    #    samples=samples.to('cuda')
                    #    feats_gt_clip = self.clipVision.encode_image(samples)
                        #feats_gt.append(image_features)
                            #text_features = model.encode_text(text)
                    #breakpoint()
                    bbox_feat_gt_conv=self.conv(bbox_feat_gt)
                    bbox_feat_gt_conv_pool=self.pool(bbox_feat_gt_conv).squeeze(2).squeeze(2)
                    features=bbox_feat_gt_conv_pool
                    #breakpoint()

                    features=F.normalize(features, p=2, dim=0)
                    
                    #breakpoint()
                    sim =features@self.class_embeddings.T
                    ...
                    target=torch.flatten(gt_label_selected)#.to('cuda')
                    #breakpoint()
                    #badones=target!=255
                    ...
                    #gt_feat_clip=torch.nn.functional.normalize(gt_feat_clip, p=2.0, dim=1)
