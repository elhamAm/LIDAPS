...
class MaskRCNNPanopticDevilInst7(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 auxiliary_head=None):
        super(MaskRCNNPanopticDevilInst7, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                roi_head=roi_head,
        ...
        if gt_bboxes:
            ...
            if not set_loss_to_zero:
                ...
                for j in range(2):
                    for i in range(gt_bboxes[j].shape[0]):
                        ...
                        if(x_tl+1<x_br and y_tl+1<y_br):
                            ...
                            if(True):#sample.shape[1] > 30 and sample.shape[2] > 30):
                ...
                if(len(samples)>0):
                    ...
                    #breakpoint()bbox_feat_gt
                    #breakpoint()
                    bbox_feat_gt=torch.nn.functional.normalize(bbox_feat_gt, p=2.0, dim=1)
