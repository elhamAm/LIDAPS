...
from .two_stage_other import TwoStageDetectorOther
...
class MaskRCNNPanopticDiffUnfrozenSimpleFPNNoProp(TwoStageDetectorOther):
    ...
    def __init__(self,
                 ...
                 auxiliary_head=None):
        super(MaskRCNNPanopticDiffUnfrozenSimpleFPNNoProp, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                simple_neck=simple_neck,
                                                rpn_head=rpn_head,
                                                roi_head=roi_head,
                                                train_cfg=train_cfg,
                                                test_cfg=test_cfg,
        ...
        #breakpoint()
        self.patt=10
        self.folder='mask_rcnn_panoptic_diff_unfrozenSimpleFPNNoProp'
        import os
    ...
    def extract_feat(self, img):
        ...
        if(not self.initialBackBone):
            ...
            #breakpoint()
            t = torch.zeros((img.shape[0],), device=img.device).long()# changed this to 0
            #self.backbone.eval()
        ...
        if gt_bboxes:
            ...
            if not set_loss_to_zero:
                ...
                if self.with_rpn:
                    ...
                    #x_b=[x_b[0].detach(), x_b[1].detach(),x_b[2].detach(), x_b[3].detach()]
                    #x_n=[x_n[0].detach(), x_n[1].detach(),x_n[2].detach(), x_n[3].detach(),x_n[4].detach()]
                    if self.with_neck:
                        #breakpoint()
                        #x_b=
                        x_simple_neck=self.simple_neck([x_b[0].detach(), x_b[1].detach(),x_b[2].detach(), x_b[3].detach()])
                        #breakpoint()
                        #x_n=[x_n[0].detach(), x_n[1].detach(),x_n[2].detach(), x_n[3].detach(),x_n[4].detach()]
                        x_n = self.neck(x_simple_neck)#outs_orig)
                ...
                roi_losses, bb = self.roi_head.forward_train(img,
                                                                x_n,
                                                                img_metas,
                                                                proposal_list,
                                                                gt_bboxes,
    ...
    def simple_test(self, img, img_meta, rescale=True, proposals=None):
        ...
        #breakpoint()
        #x_b=[x_b[0].detach(), x_b[1].detach(),x_b[2].detach(), x_b[3].detach()]
        x_simple_neck=self.simple_neck(x_b)
        #breakpoint()
        #x_n=[x_n[0].detach(), x_n[1].detach(),x_n[2].detach(), x_n[3].detach(),x_n[4].detach()]
        x_n = self.neck(x_simple_neck)#outs_orig)
