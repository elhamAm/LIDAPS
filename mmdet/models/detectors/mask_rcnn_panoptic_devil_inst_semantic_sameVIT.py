...
class MaskRCNNPanopticDevilInstSemSameVIT(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 auxiliary_head=None):
        super(MaskRCNNPanopticDevilInstSemSameVIT, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                roi_head=roi_head,
        ...
        with torch.no_grad():
            ...
            for i in range(len(labels.labels)):
                if(labels.labels[i].trainId !=  255 and labels.labels[i].trainId !=  -1):
                    ...
                    texts = texts + [clip.tokenize(template.format(labels.labels[i].name)) for template in imagenet_templates] #['a photo of a ' + labels.labels[i].name] # #format with class
                    #breakpoint()
                    text_inputs = torch.cat(texts)
                    text_inputs=text_inputs.to('cuda')
                    #breakpoint()
                    embeddings = self.clip.encode_text(text_inputs).detach().mean(dim=0)
        ...
        texts = texts + [clip.tokenize("")] #['a photo of a ' + labels.labels[i].name] # #format with class
        text_inputs = torch.cat(texts)
        text_inputs=text_inputs.to('cuda')
        embeddings = self.clip.encode_text(text_inputs).detach().mean(dim=0)
        zeroshot_weights.append(embeddings)
    ...
    def encode_decode(self, img, img_metas):
        ...
        out_decode_head,_ = self._decode_head_forward_test(x, img_metas)
    ...
    def _decode_head_forward_train(self, x, img_metas, gt_semantic_seg, seg_weight=None):
        losses = dict()
        #breakpoint()
        loss_decode = self.decode_head.forward_train(x, self.class_embeddings, img_metas, gt_semantic_seg, self.train_cfg, seg_weight)
    ...
    def forward_train(self,
        ...
        if gt_bboxes:
            ...
            if not set_loss_to_zero:
                ...
                rois_img = invTrans(rois_img)
                import torchvision.transforms.functional as TF
                IMG_MEAN = [v * 255 for v in (0.48145466, 0.4578275, 0.40821073)]
                IMG_VAR = [v * 255 for v in (0.26862954, 0.26130258, 0.27577711)]
                rois_img = TF.normalize(rois_img, IMG_MEAN, IMG_VAR)
                bbox_feat_gt_=[]
                #breakpoint()
                labels_all=[]
                for j in range(2):
                    for i in range(gt_bboxes[j].shape[0]):
                        #breakpoint()
                        y_tl, x_tl, y_br, x_br = gt_bboxes[j][i]#gt_bboxes[j][gt_labels[j]==6][i]#
                        

                        #breakpoint()
                        items=['person', 'rider', 'car', 'truck', 'bus', 'train', 'motorcycle', 'bicycle']#['synthetic car', 'real car']#
                        labels
                        if(x_tl+1<x_br and y_tl+1<y_br):
                            sample=rois_img[j,:,int(x_tl):int(x_br), int(y_tl):int(y_br)] 
                            ...
                            if(True):#sample.shape[1] > 25 or sample.shape[2] > 25):
                                ...
                                #save_image((trans_sample.squeeze(0)+2.)/2., 'trans_sample.png')
                                #feats_gt_clip = self.clip.encode_image(trans_sample.to('cuda'))
                                #breakpoint()
                                samples.append(trans_sample.squeeze(0))#trans_sample.squeeze(0))
                                #breakpoint()
                                bbox_feat_gt_.append(bbox_feat_gt[i])
                                labels_all.append(gt_labels[j][i])
                                

                                #save_image((trans_sample.squeeze(0)+2.)/2., 'sample.png')
                                #breakpoint()
                                #print('sample.shape[1]: ', sample.shape[1], 'sample.shape[2]: ', sample.shape[2])
                                #print('gtLables: ', gt_labels[j][i])
                                #breakpoint()
                                #feats_pred.append(bbox_results['bbox_feats'][i])
                #for i in range(rois.shape[0]//2,rois.shape[0]):
                #breakpoint()
                #samples=[]
                if(len(samples)>0):
                    #print('gt label: ', gt_lab)
                    #breakpoint()
                    samples=torch.stack(samples)
                    labels_all=torch.stack(labels_all)
                    bbox_feat_gt=torch.stack(bbox_feat_gt_)
                    #breakpoint()
                    ...
                    #print('clip values: ', similarity)
                    #print('items: ', items)
                    #print('top choice clip: ', items[indices[0]])
                    #import numpy as np
                    #indicesList=indices.squeeze(1).to('cpu').tolist()
                    #itemsList=np.array([items]*len(indicesList))
                    #itemsList=np.array(itemsList)
                    #preds=itemsList[:,np.array(indicesList)]
                    #breakpoint()
                    #preds=itemsList[np.arange(len(itemsList)), indicesList]
                    #breakpoint()
                    #labels_all=labels_all.to('cpu').tolist()
                    #gtLabels=itemsList[np.arange(len(itemsList)),labels_all]
                    #breakpoint()
                    correctPreds = torch.eq(labels_all,indices.squeeze(1))
                    feats_gt_clip=feats_gt_clip[correctPreds]
                    bbox_feat_gt=bbox_feat_gt[correctPreds]
                    ...
                    #bbox_feat_gt_conv_pool=torch.nn.functional.normalize(bbox_feat_gt_conv_pool, p=2.0, dim=1)
                    #feats_gt_clip=torch.nn.functional.normalize(feats_gt_clip, p=2.0, dim=1)
                    #losses['roi.loss_clip_inst'] = self.loss_clip(bbox_feat_gt_conv_pool,feats_gt_clip.to(torch.float32)).float()#*self.train_cfg.loss_weight_clip
                    losses['roi.loss_clip_inst'] = self.loss_clip(bbox_feat_gt_conv_pool,feats_gt_clip.to(torch.float32)).mean()#.float()#*100.
                    
                    #breakpoint()
                #losses.update(lossEuc)
    ...
    def simple_test(self, img, img_meta, rescale=True, proposals=None):
        ...
        #print('the shape of x_n: ', len(x_n), x_n[0].shape, x_n[1].shape, x_n[2].shape, x_n[3].shape, x_n[4].shape)
        #breakpoint()
        out_decode_head,_ = self._decode_head_forward_test(x, img_meta)
        #breakpoint()
