...
class MaskRCNNPanopticDacsTextEmbed(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 auxiliary_head=None):
        super(MaskRCNNPanopticDacsTextEmbed, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                roi_head=roi_head,
    ...
    def _decode_head_forward_train(self, x, img_metas, gt_semantic_seg, seg_weight=None):
        losses = dict()
        #breakpoint()
        loss_decode, feat_fused = self.decode_head.forward_train(x, img_metas, gt_semantic_seg, self.train_cfg, seg_weight)
        losses.update(add_prefix(loss_decode, 'decode'))
        return losses, feat_fused
    ...
    def forward_train(self,
        ...
        if gt_semantic_seg is not None:
            loss_decode, feats_fused = self._decode_head_forward_train(x, img_metas, gt_semantic_seg, seg_weight)
            losses.update(loss_decode)
            losses['feats_fused']=feats_fused
        if gt_bboxes:
