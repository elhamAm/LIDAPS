...
class MaskRCNNPanopticVitPureArtificialFPN(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 auxiliary_head=None):
        super(MaskRCNNPanopticVitPure, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                roi_head=roi_head,
    ...
    def extract_feat(self, img):
        ...
        x_backbone=x_backbone[:4]
        import torch.nn.functional as Fun
        x_backbone_new=[]
        x_backbone_new.append(Fun.interpolate(x_backbone[0], [128, 128], mode='bilinear', align_corners=True))
        x_backbone_new.append(Fun.interpolate(x_backbone[1], [64, 64], mode='bilinear', align_corners=True))
        x_backbone_new.append(Fun.interpolate(x_backbone[2], [32, 32], mode='bilinear', align_corners=True))
        x_backbone_new.append(Fun.interpolate(x_backbone[3], [16, 16], mode='bilinear', align_corners=True))
        breakpoint()
        ...
        if self.with_neck:
            #x_neck = self.neck(x_backbone_new)
            x_neck.append(Fun.interpolate(x_backbone_new[0], [128,128], mode='bilinear', align_corners=True))
            #breakpoint()

            
            #breakpoint()
            #x_neck_new=[]
            #x_neck_new.append(Fun.interpolate(x_neck[0], [128,128], mode='bilinear', align_corners=True))
            #x_neck_new.append(Fun.interpolate(x_neck[1], [64,64], mode='bilinear', align_corners=True))
            #x_neck_new.append(Fun.interpolate(x_neck[2], [32,32], mode='bilinear', align_corners=True))
            #x_neck_new.append(Fun.interpolate(x_neck[3], [16,16], mode='bilinear', align_corners=True))
            #x_neck_new.append(Fun.interpolate(x_neck[4], [8,8], mode='bilinear', align_corners=True))
            #breakpoint()
            #x_neck[1] = Fun.interpolate(x_neck[1], [64,64], mode='bilinear', align_corners=True)
            #x_neck[2] = Fun.interpolate(x_neck[2], [32,32], mode='bilinear', align_corners=True)
            #x_neck[3] = Fun.interpolate(x_neck[3], [16,16], mode='bilinear', align_corners=True)
            #breakpoint()
            #(Pdb) x_neck[0].shape
            #torch.Size([2, 256, 128, 128])
            #(Pdb) x_neck[1].shape
