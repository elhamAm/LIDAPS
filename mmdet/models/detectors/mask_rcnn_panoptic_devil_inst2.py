...
class MaskRCNNPanopticDevilInst2(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 auxiliary_head=None):
        super(MaskRCNNPanopticDevilInst2, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                roi_head=roi_head,
        ...
        if gt_bboxes:
            ...
            if not set_loss_to_zero:
                ...
                for i in range(gt_bboxes[0].shape[0]):
                    y_tl, x_tl, y_br, x_br = gt_bboxes[0][i]

                    #breakpoint()
                    if(x_tl+1<x_br and y_tl+1<y_br):
                        sample=rois_img[0,:,int(x_tl):int(x_br), int(y_tl):int(y_br)] 

                        trans_sample = Fun.interpolate(sample.unsqueeze(0), [224, 224], mode='bilinear', align_corners=True)
                        col=1
                        #rois_img[0,:,int(x_tl):int(x_tl) + 2, int(y_tl):int(y_br)] = col
                        #rois_img[0,:,int(x_br):int(x_br) + 2, int(y_tl):int(y_br)] = col
                        #rois_img[0,:,int(x_tl):int(x_br), int(y_tl):int(y_tl) + 2] = col
                        #rois_img[0,:,int(x_tl):int(x_br), int(y_br):int(y_br) + 2] = col
                        #save_image(rois_img[0,...]+4./2., './all.png')
                        #save_image(trans_sample+4./2., './sample.png')
                        #m = torch.nn.ZeroPad2d((224-sample.shape[1],0,224-sample.shape[2],0))
                        #breakpoint()
                        #trans_sample2=m(sample)
                        #save_image(trans_sample2+4./2., './sample2.png')
                        samples.append(trans_sample.squeeze(0))
                        bbox_feat_gt_.append(bbox_feat_gt[i])
                ...
                if(len(samples)>0):
                    ...
                    #print('bbox_feat_gt_conv_pool: ', bbox_feat_gt_conv_pool.shape)
                    #print('feats_gt_clip: ', feats_gt_clip.shape)
