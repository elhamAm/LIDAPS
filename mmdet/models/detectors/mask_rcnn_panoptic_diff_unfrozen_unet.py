...
class MaskRCNNPanopticDiffUnfrozenUnet(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 auxiliary_head=None):
        super(MaskRCNNPanopticDiffUnfrozenUnet, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                roi_head=roi_head,
        ...
        if(not self.initialBackBone):
            ...
            #breakpoint()
            loss_rcnn_cfg=1000
            if('rcnLoss' in self.train_cfg['rpn'].keys()):
                loss_rcnn_cfg=self.train_cfg['rpn']['rcnLoss']
            dev='cuda'

            a=self.train_cfg['rcnn']['assigner']['pos_iou_thr']
            b=self.train_cfg['rcnn']['assigner']['neg_iou_thr']
            c=self.train_cfg['rpn']['assigner']['pos_iou_thr']
            d=self.train_cfg['rpn']['assigner']['neg_iou_thr']

            #984 good
            #978
            if(a==0.5 and b==0.5 and c==0.7 and d==0.3):
                self.folder='unet2_roi0'
                #breakpoint()
            #977
            elif(c==0.9 and d==0.1 and loss_rcnn_cfg==0):
                #breakpoint()
                self.folder='unet1_roiNothing'
            elif(a==0.7 and b==0.5):
                self.folder='unet1_roi1'
            #983 bad
            elif(a==0.5 and b==0.5):
                self.folder='unet1_roi2'
            #982 bad
            elif(a==0.9 and b==0.5):
                self.folder='unet1_roi3'
            #981 good
            elif(a==0.9 and b==0.3):
                self.folder='unet1_roi4'
            #980 bad
            elif(a==0.9 and b==0.1):
                self.folder='unet1_roi5'
            #979 bad
            elif(a==0.5 and b==0.1):
                self.folder='unet1_roi6'

            
            import os
            os.makedirs('PATH_TO_FILL/elham/'+self.folder+'_train', exist_ok=True)
            os.makedirs('PATH_TO_FILL/elham/'+self.folder+'_test', exist_ok=True)
    ...
    def extract_feat(self, img):
        ...
        if(not self.initialBackBone):
            ...
            outs, outs_orig = self.unet(isEval,latents, t, c_crossattn=[c_crossattn])
            #breakpoint()
            x_backbone = outs
        else:
        ...
        if self.with_neck:
            x_neck = self.neck(outs_orig)#outs_orig)
        #breakpoint()
        ...
        if gt_bboxes:
            ...
            if not set_loss_to_zero:
                ...
                if(self.count%10000 == 0):
                    save_image((img[0]+1.)/2., 'PATH_TO_FILL/elham/'+self.folder+'_train/'+name)
                #breakpoint()
                ...
                from torchvision.utils import save_image
                if(self.count%10000 == 0):
                    save_image((rpn_img[0]+1.)/2., 'PATH_TO_FILL/elham/'+self.folder+'_train/'+name)

                name=str(self.count)+'_gt_bb_train.png'
                if(self.count%100000 == 0):
                    save_image((gt_img[0]+1.)/2., 'PATH_TO_FILL/elham/'+self.folder+'_train/'+name)


                name=str(self.count)+'_roi_train.png'
                if(self.count%100000 == 0):
                    save_image((roi_img[0]+1.)/2., 'PATH_TO_FILL/elham/'+self.folder+'_train/'+name)
    ...
    def simple_test(self, img, img_meta, rescale=True, proposals=None):
        ...
        if(self.test_count%50==0):
            save_image((img[0]+1.)/2., 'PATH_TO_FILL/elham/'+self.folder+'_test/'+name)
        #breakpoint()
        for i in range(proposal_list[0].shape[0]):
            col =100
            y_tl, x_tl, y_br, x_br,_ = proposal_list[0][i]
        ...
        from torchvision.utils import save_image
        if(self.test_count%50==0):
            save_image((rpn_img+1.)/2., 'PATH_TO_FILL/elham/'+self.folder+'_test/'+name)
        ################################# save eval image
        ...
        if(self.test_count%50==0):
            save_image((roi_img+1.)/2., 'PATH_TO_FILL/elham/'+self.folder+'_test/'+name)
