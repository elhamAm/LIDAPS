...
class MaskRCNNPanopticDiff(TwoStageDetector):
    ...
    def __init__(self,
        ...
        if(not self.initialBackBone):
            # get the unet ready
            sd_path='PATH_TO_FILL/v1-5-pruned.ckpt'#'./stable-diffusion/models/ldm/stable-diffusion-v1/model.ckpt'
    ...
    def extract_feat(self, img):
        ...
        if(not self.initialBackBone):
            ...
            with torch.no_grad():
                ...
                #c_crossattn = eslf.text_adapter(latents, self.class_embeddings, self.gamma) #torch.cat([self.class_embeddings.unsqueeze(0),self.class_embeddings.unsqueeze(0)],dim=0)## NOTE: here the c_crossattn should be expand_dim as latents
