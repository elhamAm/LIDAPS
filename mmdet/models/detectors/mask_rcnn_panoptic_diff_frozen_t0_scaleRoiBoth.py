...
class MaskRCNNPanopticDiffFrozenT0ScaleRoiBoth(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 auxiliary_head=None):
        super(MaskRCNNPanopticDiffFrozenT0ScaleRoiBoth, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                roi_head=roi_head,
        ...
        if(not self.initialBackBone):
            ...
            self.folder='diff_roi_hyperbumble_t0_testHypers_RoiScale_frozen_boththres_rpn'
    ...
    def extract_feat(self, img):
        ...
        if(not self.initialBackBone):
            ...
            with torch.no_grad():
                ...
                outs[0] = Fun.interpolate(outs[0], [64*2, 64*2], mode='bilinear', align_corners=True)
                outs[1] = Fun.interpolate(outs[1], [32*2, 32*2], mode='bilinear', align_corners=True)
                outs[2] = Fun.interpolate(outs[2], [16*2, 16*2], mode='bilinear', align_corners=True)
                outs[3] = Fun.interpolate(outs[3], [8*2, 8*2], mode='bilinear', align_corners=True)
                #breakpoint()
                #outs
