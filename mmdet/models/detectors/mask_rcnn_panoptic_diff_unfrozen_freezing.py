...
class MaskRCNNPanopticDiffUnfrozenFreezing(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 test_cfg,
                 neck=None,
                 simple_neck=None,                 pretrained=None,
                 init_cfg=None,
                 use_neck_feat_for_decode_head=False,
                 auxiliary_head=None):
        super(MaskRCNNPanopticDiffUnfrozenFreezing, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                roi_head=roi_head,
    ...
    def extract_feat(self, img):
        ...
        if(not self.initialBackBone):
            ...
            #breakpoint()
            t = torch.ones((img.shape[0],), device=img.device).long()#889 torch.zeros((img.shape[0],), device=img.device).long()#changed this to 0
            #self.backbone.eval()
