...
class MaskRCNNPanopticPODA(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 auxiliary_head=None):
        super(MaskRCNNPanopticPODA, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                roi_head=roi_head,
        ...
        import clipOrig
        model_path = clipOrig._download(model_url, os.path.expanduser("PATH_TO_FILL/cache/clip"))
        device = torch.device('cuda' if torch.cuda.is_available() else "cpu")
        with open(model_path, 'rb') as opened_file:
            backbone = torch.jit.load(opened_file, map_location="cpu").eval()
            replace_stride_with_dilation=[False,False,True]
            self.backbone = clipOrig.build_model(backbone.state_dict(),replace_stride_with_dilation=replace_stride_with_dilation).to(device)
    ...
    def extract_feat(self, img):
        ...
        breakpoint()
        x_backbone = self.backbone(img, trunc1=False,trunc2=False,trunc3=False,trunc4=False,get1=True,get2=False,get3=False,get4=False)
        breakpoint()
