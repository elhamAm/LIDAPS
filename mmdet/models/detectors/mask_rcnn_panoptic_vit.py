...
class DenseCLIP_MaskRCNN_VIT(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 **args):
        super(DenseCLIP_MaskRCNN_VIT, self).__init__(init_cfg)
        ...
        self.gamma = nn.Parameter(torch.ones((2,19,512)) * 1e-4)
    ...
    def extract_feat(self, img):
        ...
        #breakpoint()
        #(Pdb) x[0].shape
        #torch.Size([2, 768, 160, 160])
        #(Pdb) x[1].shape
        #torch.Size([2, 768, 80, 80])
        #(Pdb) x[2].shape
        #torch.Size([2, 768, 40, 40])
        #(Pdb) x[3].shape
        #torch.Size([2, 768, 20, 20])
        return x
    ...
    def after_extract_feat(self, x):
        ...
        # (B, K, C)
        #breakpoint()
        text_embeddings = self.gamma.to('cuda') * text_diff# + self.texts.to(global_feat.device) 
        ...
        #x_orig[2] = torch.cat([x_orig[2], score_map], dim=1)
        #breakpoint()
