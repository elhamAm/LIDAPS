...
class MaskRCNNPanopticResnetPureUnfrozen(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 auxiliary_head=None):
        super(MaskRCNNPanopticResnetPureUnfrozen, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                roi_head=roi_head,
        ...
        print('FINISHED THIS')
        import os
        self.folder='orig_instOnly_resnetPure'
        os.makedirs('PATH_TO_FILL/'+self.folder+'_train', exist_ok=True)
        os.makedirs('PATH_TO_FILL/'+self.folder+'_test', exist_ok=True)

        #checkpoint='PATH_TO_FILL/pretrained/RN101.pt'
        #self.backbone.load_state_dict(checkpoint, map_location=device)
        #self.backbone = torch.jit.load(checkpoint, map_location="cpu").eval()
        #breakpoint()
