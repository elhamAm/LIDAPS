...
class MaskFormer(SingleStageDetector):
    ...
    def __init__(self,
        ...
        cityscapes_thing_list=[11, 12, 13, 14, 15, 16, 17, 18]#self.panoptic_head.cityscapes_thing_list
    ...
    def forward_train(self, img, img_metas, gt_bboxes, gt_labels,  gt_masks, gt_semantic_seg=None,  gt_bboxes_ignore=None, return_feat=False, update_output_debug=False, **kargs):
        ...
        for i in range(gt_bboxes[0].shape[0]):
            ...
            #print('imssssss: ', img)
            bb_img[0,:,x_tl:x_tl + 2, y_tl:y_br] = col
            bb_img[0,:,x_br:x_br + 2, y_tl:y_br] = col
            bb_img[0,:,x_tl:x_br, y_tl:y_tl + 2] = col
            bb_img[0,:,x_tl:x_br, y_br:y_br + 2] = col
        
        name=str(self.count)+'_bb_train.png'
        if(self.count%self.patt == 0):
            save_image((bb_img[0]+1.)/2., 'PATH_TO_FILL/'+self.folder+'_train/'+name)
