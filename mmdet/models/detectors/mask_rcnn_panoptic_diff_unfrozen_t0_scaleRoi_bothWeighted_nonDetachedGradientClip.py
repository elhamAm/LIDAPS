...
class MaskRCNNPanopticDiffUnfrozenT0ScaleRoiBothWeightedNonDetachedGradientClip(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 auxiliary_head=None):
        super(MaskRCNNPanopticDiffUnfrozenT0ScaleRoiBothWeightedNonDetachedGradientClip, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                roi_head=roi_head,
    ...
    def train_step(self, data, optimizer):
        ...
        if('rpn.loss_rpn_cls' in losses):
            ...
            loss.backward()
            #breakpoint()
            torch.nn.utils.clip_grad_norm(self.parameters(), max_norm=0.1, norm_type=2.0)

            self.maxGrad=[]
            for param in self.parameters():
                #print(param.grad)
                if param.grad is not None:
                    self.maxGrad.append(torch.max(torch.abs(param.grad)))
            #print('max gradient: ', max(self.maxGrad))
            #breakpoint()
