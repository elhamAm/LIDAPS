...
class MaskRCNNPanopticDevilInst4NoCheckSizeClipSimilMeanPoolMask(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 auxiliary_head=None):
        super(MaskRCNNPanopticDevilInst4NoCheckSizeClipSimilMeanPoolMask, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                roi_head=roi_head,
        ...
        self.pool = torch.nn.AvgPool2d(14)
        ...
        if gt_bboxes:
            ...
            if not set_loss_to_zero:
                ...
                if(len(samples)>0):
                    ...
                    #breakpoint()
                    #bbox_feat_gt_conv_pool=torch.nn.functional.normalize(bbox_feat_gt, p=2.0, dim=1)
                    ...
                    #print(1-self.loss_clip(bbox_feat_gt_conv_pool,feats_gt_clip.to(torch.float32)).mean())#.float()#*100.)
                    losses['roi.loss_clip_inst'] = 1-self.loss_clip(bbox_feat_gt_conv_pool,feats_gt_clip.to(torch.float32)).mean()#.float()#*100.
