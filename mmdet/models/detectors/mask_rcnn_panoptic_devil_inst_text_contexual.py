...
class MaskRCNNPanopticDevilInstTextContexual(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 auxiliary_head=None):
        super(MaskRCNNPanopticDevilInstTextContexual, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                roi_head=roi_head,
        ...
        text_dim=768#self.class_embeddings.shape[1]#self.class_embeddings.shape
        self.gamma = torch.nn.Parameter(torch.ones(text_dim) * 1e-4)
        #breakpoint()
        
        
        device = "cuda" if torch.cuda.is_available() else "cpu"
        self.clip, self.preprocess = clip.load("ViT-B/32", device=device)   
        self.conv = torch.nn.Conv2d(256, 768, 1, stride=1).to('cuda')
        ...
        from mmdet.models.detectors.contextDecoder import AttentionPool2d
        input_resolution=512
        width=64
        embed_dim = width * 32
        output_dim=768
        self.attnpool = AttentionPool2d(input_resolution // 32, 256, 32, output_dim)

        self.clip, self.preprocess = clip.load("ViT-B/32", device=device)  


            
        from mmdet.models.detectors.contextDecoder import ContextDecoder
    ...
    def forward_train(self,
        ...
        #print('#################################################################################')
        my_labels=['person', 'rider', 'car', 'truck', 'bus', 'train', 'motorcycle', 'bicycle']
        #with torch.no_grad():
        text_inputs = torch.cat([clip.tokenize(f"a photo of a {c}") for c in my_labels]).to('cuda')
        class_embeddings = self.clip.encode_text(text_inputs)
        breakpoint()
        if self.use_neck_feat_for_decode_head:
            assert self.with_neck, 'self.with_neck is False, so set the self.use_neck_feat_for_decode_head to False.'
        ...
        if gt_bboxes:
            ...
            if not set_loss_to_zero:
                ...
                #breakpoint()
                global_feat, visual_embeddings=self.attnpool(x_n[4])
                #(Pdb) visual_embeddings.shape
                #torch.Size([2, 1024, 8, 8])
                #(Pdb) global_feat.shape
                #torch.Size([2, 1024])
                ...
                class_embeddings=class_embeddings.expand(B, -1, -1)
                text_diff = self.context_decoder(class_embeddings, visual_context)
                #breakpoint()
                class_embeddings = class_embeddings + self.gamma * text_diff
                ...
                for j in range(2):
                    for i in range(gt_bboxes[j].shape[0]):
                        y_tl, x_tl, y_br, x_br = gt_bboxes[j][i]

                        #breakpoint()
                        if(x_tl+1<x_br and y_tl+1<y_br):
                            sample=img[j,:,int(x_tl):int(x_br), int(y_tl):int(y_br)] 
                            
                            if(sample.shape[1] > 25 and sample.shape[2] > 25):
                                #breakpoint()
                                gt_feat_clip.append(class_embeddings[0][gt_labels[j][i]])
                                #samples.append(trans_sample.squeeze(0))
                                bbox_feat_gt_.append(bbox_feat_gt[i])
                                #feats_pred.append(bbox_results['bbox_feats'][i])
                #for i in range(rois.shape[0]//2,rois.shape[0]):
                #breakpoint()
                #samples=[]
                if(len(bbox_feat_gt_)>0):
                    gt_feat_clip=torch.stack(gt_feat_clip)
                    ...
                    losses['roi.loss_clip_inst'] = self.loss_clip(bbox_feat_gt_conv_pool,gt_feat_clip.to(torch.float32)).float()#*self.train_cfg.loss_weight_clip
                    #breakpoint()
