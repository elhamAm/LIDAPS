...
class MaskRCNNPanopticTextRefineContext(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 auxiliary_head=None):
        super(MaskRCNNPanopticTextRefineContext, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                roi_head=roi_head,
        ...
        if(denseClip):
            ...
            self.clip=build_model(state_dict).float().to('cuda').eval()#.visual
        #breakpoint()
        self.attnPool=AttentionPool2dDense(spacial_dim=16, embed_dim=512, num_heads=8, output_dim=512)
        self.context_decoder=ContextDecoder(context_length=16, transformer_width=256, transformer_heads=4,transformer_layers=6,visual_dim=512,dropout=0.1,outdim=512)
    ...
    def extract_feat(self, img):
        ...
        #breakpoint()
        with torch.no_grad():
            text_embeddings = self.clip.encode_text(texts).expand(B, -1, -1)
        #breakpoint()
