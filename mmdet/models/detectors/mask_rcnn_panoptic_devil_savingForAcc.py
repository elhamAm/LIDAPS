...
class MaskRCNNPanopticDevilSaveAcc(TwoStageDetector):
    ...
    def __init__(self,
                 ...
                 auxiliary_head=None):
        super(MaskRCNNPanopticDevilSaveAcc, self).__init__(
                                                backbone=backbone,
                                                neck=neck,
                                                rpn_head=rpn_head,
                                                roi_head=roi_head,
    ...
    def simple_test(self, img, img_meta, rescale=True, proposals=None):
        ...
        if(self.test_count%self.patt==0):
        ...
        import numpy as np
        #breakpoint()
        np.save('PATH_TO_FILL/jobs/dumpMasks/image_'+names.split('_')[0]+'.npy', np.asarray(inst_pred, dtype=object))
        self.iter_test+=1
        boxes=inst_pred[0][0]
        masks=inst_pred[0][1]
