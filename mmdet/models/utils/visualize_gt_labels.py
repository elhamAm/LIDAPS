...
def subplotimg(ax, img, title, range_in_title=False, palette=Cityscapes_palette, drawboxes=False, bboxes=None, **kwargs):
    if img is None:
        return
    ...
    if drawboxes:
        ...
        num_bxs = bboxes.shape[0]
        for bid in range(num_bxs):
            box = bboxes[bid]
            x1 = box[0]
            y1 = box[1]
            x2 = box[2]
            ...
            w = x2 - x1 + 1
            h = y2 - y1 + 1
            rect = patches.Rectangle((x1, y1), w, h, linewidth=2, edgecolor='#ff3000ff', facecolor='none')
            ax.add_patch(rect)
...
def subplotInstSeg(img, masks):
    _, ih, iw = img.shape
    inst_map = np.zeros((ih, iw), dtype=np.int)
    num_instances = masks.shape[0]
    num_colors = num_instances
    colormap = np.zeros((num_colors+1, 3), dtype=np.uint8)
    colormap[0, :] = np.array([0, 0, 0]) # for background class
    for i in range(num_colors):
        inst_map[masks[i] == 1] = i+1
        colormap[i+1, :] = random_color(rgb=True, maximum=255)
    colored_label = colormap[inst_map]
    # colored_label = 0.5 * colored_label + (1 - 0.5) * img
    return colored_label.astype(int)

def visualize_mask_rcnn_gt_labels(img, img_metas, gt_bboxes, gt_labels, gt_masks, gt_bboxes_ignore):
    '''
    ...
    import os
    # img, img_metas, gt_bboxes, gt_labels, gt_masks, gt_bboxes_ignore

    means, stds = get_mean_std(img_metas, 'cuda:0')
    vis_img = torch.clamp(denorm(img, means, stds), 0, 1)
    out_dir = 'PATH_TO_FILL/Downloads'
    batch_size = 1
