...
@UDA.register_module()
class DACSMixed(UDADecorator):

    def __init__(self, **cfg):
        super(DACSMixed, self).__init__(**cfg)
        self.local_iter = 0
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, # daformer args
        ...
        if(apply):
            ...
            t0=copy.deepcopy(gt_bboxes[0][gt_labels[0]==2])
            t1=copy.deepcopy(gt_bboxes[1][gt_labels[1]==2])
            if(t0.shape[0] > 0 or t1.shape[0] > 0):
                gt_masks_torch0=torch.tensor(gt_masks[0].masks).to('cuda')[gt_labels[0]==2]
                gt_masks_torch1=torch.tensor(gt_masks[1].masks).to('cuda')[gt_labels[1]==2]
                ...
                if(visual):
                    ...
                    for q in range(gt_bboxes_inst[0].shape[0]):
                        #breakpoint()
                        if(gt_labels_inst[0][q]==2):
                            col=0
                            y_tl, x_tl, y_br, x_br = gt_bboxes_inst[0][q]#proposal_list[0][i]
                            x_tl = int(x_tl)
                            y_tl = int(y_tl)
                            x_br = int(x_br)
                            y_br = int(y_br)
                            ...
                            #print('imssssss: ', img)
                            img_pastes_1[0,:,x_tl:x_tl + 2, y_tl:y_br] = col
                            img_pastes_1[0,:,x_br:x_br + 2, y_tl:y_br] = col
                            img_pastes_1[0,:,x_tl:x_br, y_tl:y_tl + 2] = col
                            img_pastes_1[0,:,x_tl:x_br, y_br:y_br + 2] = col
                    save_image((img_pastes_1[0]+2.)/2., './carsBoxes.png')
                    #breakpoint()
                    masks=torch.tensor(gt_masks_inst[0].masks).to('cuda')
                    #breakpoint()
                    #t=-100
                    for q in range(masks.shape[0]):
                        #breakpoint()
                        if(gt_labels_inst[0][q]==2):
                            img_pastes_2[0]=(1-(masks[q]).unsqueeze(0).repeat(3,1,1))*img_pastes_2[0]
                            #t-=20
                    save_image((img_pastes_2[0]+2)/2., './carsMasks.png')
                    #breakpoint()
                #breakpoint()
                #print('here')
