        ...
        if(apply):
            ...
            if(True in list(conds0) or True in list(conds1)):
                ...
                for j in range(2):
                    ...
                    gt_bboxes_inst1_new=[]
                    gt_labels_inst1_new=[]
                    for masksi in range(gt_masks_inst[j].masks.shape[0]-1,-1,-1):
                        #labels_stacked_0[masksi]
                        #breakpoint()
                        #print('masksi: ', masksi)
                        labels_stacked[j][gt_masks_inst[j].masks[masksi]==1]+=1
                        #breakpoint()
                        if(labels_stacked[j][labels_stacked[j]>1].shape[0]>0):
                            #breakpoint()
                            #change mask
                            masks01[j][masksi][labels_stacked[j]>1]=0
                            if(torch.unique(masks01[j][masksi]).shape[0]>1):
                                
                            
                                #breakpoint()
                                
                                #gt_masks_inst[1][masksi]=BitmapMasks(masks.cpu().numpy(),mask.shape[0],mask.shape[1])
                                #change box
                                #breakpoint()
                                #breakpoint()
                                minIndx=np.min(np.argwhere(masks01[j].cpu().numpy()[masksi]>0)[:, 0])
                                maxIndx=np.max(np.argwhere(masks01[j].cpu().numpy()[masksi]>0)[:, 0])

                                minIndy=np.min(np.argwhere(masks01[j].cpu().numpy()[masksi]>0)[:, 1])
                                maxIndy=np.max(np.argwhere(masks01[j].cpu().numpy()[masksi]>0)[:, 1])

                                #breakpoint()
                                gt_bboxes_inst[j][masksi][0]=minIndy
                                gt_bboxes_inst[j][masksi][2]=maxIndy
                                gt_bboxes_inst[j][masksi][1]=minIndx
                                gt_bboxes_inst[j][masksi][3]=maxIndx
                        ...
                        if(torch.unique(masks01[j][masksi]).shape[0]>1):
                            ...
                            gt_masks_inst1_new.append(masks01[j][masksi].unsqueeze(0))
                            gt_bboxes_inst1_new.append(gt_bboxes_inst[j][masksi].unsqueeze(0))
                            gt_labels_inst1_new.append(gt_labels_inst[j][masksi].unsqueeze(0))
                        else:
                            delete=True
                            
                    #breakpoint()
                    if(len(gt_masks_inst1_new) > 0):
                        gt_masks_inst[j]= torch.cat(gt_masks_inst1_new)
                        gt_bboxes_inst[j]=torch.cat(gt_bboxes_inst1_new)
                        gt_labels_inst[j]=torch.cat(gt_labels_inst1_new)
                    else:
                        gt_masks_inst[j]= torch.tensor(gt_masks_inst1_new)
                        gt_bboxes_inst[j]=torch.tensor(gt_bboxes_inst1_new)
                        gt_labels_inst[j]=torch.tensor(gt_labels_inst1_new)

                    #breakpoint()
                            
                    gt_masks_inst[j]=BitmapMasks(gt_masks_inst[j].cpu().numpy(),gt_masks_inst[j].shape[1], gt_masks_inst[j].shape[2])
                ...
                if(visual):
                    ...
                    for j in range(2):
                        for q in range(gt_bboxes_inst[j].shape[0]):
                            
                            if(True):#if(gt_labels_inst[1][q]==2):
                                #breakpoint()
                                col=0
                                y_tl, x_tl, y_br, x_br = gt_bboxes_inst[j][q]#proposal_list[0][i]
                                ...
                                #print('imssssss: ', img)
                                img_pastes_1[j,:,x_tl:x_tl + 2, y_tl:y_br] = col
                                img_pastes_1[j,:,x_br:x_br + 2, y_tl:y_br] = col
                                img_pastes_1[j,:,x_tl:x_br, y_tl:y_tl + 2] = col
                                img_pastes_1[j,:,x_tl:x_br, y_br:y_br + 2] = col
                        save_image((img_pastes_1[j]+2.)/2., './carsBoxes'+str(j)+'.png')
                        #breakpoint()
                        #breakpoint()
                        masks=torch.tensor(gt_masks_inst[j].masks).to('cuda')
                        #breakpoint()
                        ...
                        for q in range(masks.shape[0]):
                            #breakpoint()
                            if(gt_labels_inst[j][q]==2):
                                img_pastes_2[j]=(1-(masks[q]).unsqueeze(0).repeat(3,1,1))*img_pastes_2[j]#img_pastes_2[1]
                                #s+=20
                        save_image((img_pastes_2[j]+2)/2., './carsMasks'+str(j)+'.png')
                                #breakpoint()
                #breakpoint()
                #print('here')
                ...
                #breakpoint()
                #print('inst roi loss: ', inst_losses['inst.roi.loss_bbox'])
                inst_loss.backward()
