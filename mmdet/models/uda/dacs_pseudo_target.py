...
@UDA.register_module()
class DACSPSTARGET(UDADecorator):

    def __init__(self, **cfg):
        super(DACSPSTARGET, self).__init__(**cfg)
        self.local_iter = 0
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, # daformer args
        ...
        for j in range(2):
            ...
            for c in range(8):
                ...
                #breakpoint()
                nums=out_roi_head[j][0][c][out_roi_head[j][0][c][:, 4]>0.6,:4].shape[0]
                if(nums>0):
                    boxesOneset.append(torch.tensor(out_roi_head[j][0][c][out_roi_head[j][0][c][:, 4]>0.6,:4]))
                    
                    #masks out_roi_head[j][1]

                    masksOneSet.append(torch.tensor(np.array(out_roi_head[j][1][c])[out_roi_head[j][0][c][:, 4]>0.6]))
                    nums=out_roi_head[j][0][c][out_roi_head[j][0][c][:, 4]>0.6,:4].shape[0]
