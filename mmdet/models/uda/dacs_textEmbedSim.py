...
@UDA.register_module()
class DACSTextEmbedSim(UDADecorator):

    def __init__(self, **cfg):
        super(DACSTextEmbedSim, self).__init__(**cfg)
        self.local_iter = 0
    ...
    def calc_feat_dist(self, img, gt, feat=None, feats_decoderSemantic=None):
        ...
        if self.fdist_classes is not None:
            ...
            gt_selected=gt_rescaled[fdist_mask]
            ...
            #fdist_mask_= torch.permute(fdist_mask_, (1, 0, 2, 3))
            #breakpoint()
            ...
            sim =self.class_embeddings@features
            soft_score=sim
            input=soft_score.permute(1,0)#.to('cuda')
            target=torch.flatten(gt_selected)#.to('cuda')
            #breakpoint()
            #badones=target!=255
            #print(badones.device)
            #target=target.where(badones, torch.tensor(19).to('cuda:0'))
            loss_clip=loss_cls(input,gt_selected)
