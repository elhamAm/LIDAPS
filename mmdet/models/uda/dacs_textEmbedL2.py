...
@UDA.register_module()
class DACSTextEmbedL2(UDADecorator):

    def __init__(self, **cfg):
        super(DACSTextEmbedL2, self).__init__(**cfg)
        self.local_iter = 0
    ...
    def calc_feat_dist(self, img, gt, feat=None, feats_decoderSemantic=None):
        ...
        if self.fdist_classes is not None:
            ...
            scale_factor = gt.shape[-1] // feats_decoderSemantic.shape[-1]
            gt_rescaled = downscale_label_ratio(gt, scale_factor, self.fdist_scale_min_ratio, self.num_classes, 255).long().detach()
            fdist_mask = torch.any(gt_rescaled[..., None] == fdclasses, -1)

            #breakpoint()

            #fdist_mask_=fdist_mask.repeat(1,768,1,1)
            #feats_decoderSemantic[fdist_mask_==False] = 0
            gt_rescaled[gt_rescaled==6]=9
            gt_rescaled[gt_rescaled==7]=10
            gt_rescaled=gt_rescaled-9
            #gt_rescaled[gt_rescaled!=1000]=0
            gt_rescaled=gt_rescaled.permute(1,0,2,3).unsqueeze(0)
            _,_,b, w,h = gt_rescaled.shape
            _, embedSize=self.class_embeddings.shape
            #breakpoint()
            with torch.no_grad():
                gt_rescaled=gt_rescaled.repeat(embedSize,1,1,1,1)
                gt_rescaled=gt_rescaled[:,:,fdist_mask.squeeze(1)]

                #breakpoint()
                
                class_embeddings_=self.class_embeddings.unsqueeze(2).unsqueeze(3).unsqueeze(4)
                class_embeddings_=class_embeddings_.repeat(1,1,b, w, h)
                class_embeddings_=class_embeddings_.permute(1,0,2,3,4)
                class_embeddings_=class_embeddings_[:,:,fdist_mask.squeeze(1)]

                feat_gt=torch.gather(class_embeddings_, 1, gt_rescaled)
                #for i in range(10):
                #    if(torch.equal(self.class_embeddings[i,:],feat_gt[:,0,1500])):
                #        print('found')
                #breakpoint()

                features = torch.permute(feats_decoderSemantic, (1, 0, 2, 3))
                features = features[:,fdist_mask.squeeze(1)]

            feat_gt = feat_gt - features
            feat_gt = torch.norm(feat_gt, dim=1, p=2)

            feat_gt=torch.mean(feat_gt)

            #feat_dist = self.masked_feat_dist(feat[lay], feat_imnet[lay], fdist_mask)
            #self.debug_fdist_mask = feat_gt
            #self.debug_gt_rescale = gt_rescaled
        #else:
            #feat_dist = self.masked_feat_dist(feat[lay], feat_imnet[lay])
        #feat_dist = self.fdist_lambda * loss_clip #feat_dist
        feat_dist = feat_gt
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, # daformer args
        ...
        src_feats_fused = clean_losses.pop('feats_fused')
        #breakpoint()
        ...
        if self.enable_fdist:
            #breakpoint()
            feat_loss, feat_log = self.calc_feat_dist(img, gt_semantic_seg, src_feat, src_feats_fused)
            del src_feat
