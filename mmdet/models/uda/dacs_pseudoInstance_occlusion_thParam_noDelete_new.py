...
from mmseg.utils.visualize_pred import prep_ins_for_vis_mine
...
@UDA.register_module()
class DACSPSOCCPARAMNODELETENEW(UDADecorator):

    def __init__(self, **cfg):
        super(DACSPSOCCPARAMNODELETENEW, self).__init__(**cfg)
        self.local_iter = 0
    ...
    def runOtherDirection(self,thres1, thres2, img, target_img, target_img_metas, gt_bboxes, gt_masks, gt_labels, batch_size, img_metas, mixed_img2, mix_masks2):
        ...
        if(visual and not passThis):
            ...
            self.iter+=1
            #img_new1, img_new2, img_new_target1, img_new_target2=prep_ins_for_vis(img_insts_thres1[j].cpu(),  mode='GT', dataset_name='synthia', debug=False, blend_ratio=0.7, img=, show_only_person=False, sem_id=None)
        #breakpoint()
        #print('here')
        #if(True in
    def runInstances(self,thres1, thres2, img, target_img, target_img_metas, gt_bboxes, gt_masks, gt_labels, batch_size, img_metas, mixed_img2, mix_masks2):
        ...
        if(visual and not passThis):
            ...
            mix_instance_img_thre2.append(copy.deepcopy(img[0]))
            mix_instance_img_thre2.append(copy.deepcopy(img[1]))
            ...
            for q in range(masks_forVis_corrected[j].shape[0]):
                ...
                if(conf_forVis_corrected[j][q] > thres1):#0.5
                    ...
                    if q<(masks_forVis_corrected[j].shape[0]-q_true[j]):
                        #breakpoint()
                        mix_instance_img_thre1[j]=masks_forVis_corrected[j][q].repeat(3,1,1)*target_img[j]+(1-masks_forVis_corrected[j][q].repeat(3,1,1))*mix_instance_img_thre1[j][j]
            ...
            trans = transforms.Compose([ transforms.Normalize(mean = [ mean[0], mean[1], mean[2] ],
                                                     std = [ std[0], std[1], std[2] ]) ])
            mix_instance_img_thre2[j] = trans(mix_instance_img_thre2[j])
            #breakpoint()
