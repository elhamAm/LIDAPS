...
@UDA.register_module()
class DACSPSOCC08SIZEPLACEROLL(UDADecorator):

    def __init__(self, **cfg):
        super(DACSPSOCC08SIZEPLACEROLL, self).__init__(**cfg)
        self.local_iter = 0
                ...
                for c in classes:
                    ...
                    for m in range(len(masks_c)):
                        ...
                        if mask_score >= mask_score_th and np.unique(masks_c[m]).shape[0]>1:
                            ...
                            #breakpoint()
                            numOfOnes=masks_c[m][masks_c[m]==1].shape[0]
                            #print('the size of the mask: ', numOfOnes)
                            #6000 divided by 2 is okay not more
                            #457 should be divide by one
                            #1600 divide by 2 is still okay
                            #577 divide by one
                            #857 divide by 2 is good
                            big_img_mixed_instance=img_mixed_instance*(1-torch.tensor(masks_c[m])*1.).to('cuda') +  target_img[j]*torch.tensor(masks_c[m]).to('cuda')
                            if(visual):
                                save_image((big_img_mixed_instance+2.)/2., 'big_img_mixed_instance_small.png')
                                save_image(torch.tensor(masks_c[m]).float(), 'mask_big.png')

                            maxIndx=np.max(np.argwhere(masks_c[m]>0)[:, 0])
                            #minIndy=np.min(np.argwhere(masks_c[m]>0)[:, 2])
                            maxIndy=np.max(np.argwhere(masks_c[m]>0)[:, 1])

                            moveX=random.randint(0, 511-maxIndx)
                            moveY=random.randint(0, 511-maxIndy)
                            
                            maskTemp = torch.roll(torch.tensor(masks_c[m]).to('cuda'), (moveX,moveY),(0,1)).float()
                            imgTemp = torch.roll(torch.tensor(target_img[j]).to('cuda'), (moveX,moveY),(1,2)).float()
                            
                            #breakpoint()
                            if(visual):
                                save_image(maskTemp, 'mask_small_moved.png')
                            #breakpoint()
                            masks_c[m]=maskTemp.cpu().numpy()

                            
                            w=masks_c[m].shape[0]
                            h=masks_c[m].shape[1]
                            divs=[i for i in range(1,int(numOfOnes/2500)+1)]
                            #breakpoint()
                            if(divs == []):
                                divs=[1]
                            div=1/random.randint(1,len(divs))
                            #print('div: ', div)
                            new_w_h=int(w*div)
                            #masks_c[m] = Fun.interpolate(torch.tensor(masks_c[m]).to('cuda'), [masks_c[m].shape[0]//2, masks_c[m].shape[1]//2], mode='bilinear', align_corners=True)
                            maskTemp= Fun.interpolate(torch.tensor(masks_c[m]).to('cuda').unsqueeze(0).unsqueeze(0)*1., [new_w_h, new_w_h], mode='bilinear', align_corners=True)
                            #roll the mask here
                            #breakpoint()
                            #minIndx=np.min(np.argwhere(masks_c[m]>0)[:, 1])
             

                            imgTemp = Fun.interpolate(imgTemp.to('cuda').unsqueeze(0), [new_w_h, new_w_h], mode='bilinear', align_corners=True)
                            num1=int((512-new_w_h)/2)
                            num2=512-new_w_h-num1
                            #breakpoint()
                            maskTemp = Fun.pad(maskTemp, (num1,num2,num1,num2), "constant", 0) 
                            imgTemp = Fun.pad(imgTemp, (num1,num2,num1,num2), "constant", 0) 
                            #breakpoint()
                            masks_c[m]=maskTemp.squeeze(0).squeeze(0).cpu().numpy()#torch.tensor()
                            
                            if(visual):
                                save_image(maskTemp, 'mask_small.png')
                            #breakpoint()
                            #breakpoint()
                            ...
                            new_labels.append(c)
                            #breakpoint()
                            img_mixed_instance=img_mixed_instance*(1-maskTemp.squeeze(0)) +  imgTemp.squeeze(0)*maskTemp.squeeze(0)
                            if(visual):
                                save_image((img_mixed_instance+2.)/2., 'small_img_mixed_instance_small.png')
                            #breakpoint()
                            #save_image(big_img_mixed_instance, 'big_img_mixed_instance_small.png')
                            
                mix_instance_img.append(img_mixed_instance)
                if(len(new_boxes) ==0):
                    passThis=True
                    break
                #breakpoint()
                ...
                for masksi in range(masks_mixed[j].shape[0]-1,-1,-1):
                    ...
                    if(masksi >= gt_bboxes[j].shape[0]):
                        ...
                        if(not(labels_stacked_[j][labels_stacked_[j]>1].shape[0]>0)):
                            ...
                            #breakpoint()
                            if(percentage<60 and torch.unique(masks_mixed[j][masksi]).shape[0]>1):
