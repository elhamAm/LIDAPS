...
@UDA.register_module()
class DACSPSOCC08SCORENOFILTER(UDADecorator):

    def __init__(self, **cfg):
        super(DACSPSOCC08SCORENOFILTER, self).__init__(**cfg)
        self.local_iter = 0
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, # daformer args
        ...
        for j in range(2):
            ...
            if(not passThis):
                ...
                #breakpoint()
                scores_mixed.append(torch.cat([torch.tensor(gt_bboxes[j].shape[0]*[1.]).to('cuda'), torch.tensor(new_scores).to('cuda')]))
                #breakpoint()
            ...
            for j in range(2):
                ...
                for masksi in range(masks_mixed[j].shape[0]-1,-1,-1):
                    ...
                    if(masksi >= gt_bboxes[j].shape[0]):
                        ...
                        scores_mixed_corrected[j].append(scores_mixed[j][masksi])
                    else:
                ...
                scores_mixed_corrected[j]=torch.stack(scores_mixed_corrected[j])
                #targetScores.append(torch.cat(scoresOneSet).to('cuda'))
                #mixed_each_score[j]=torch.stack(mixed_each_score[j])
        ...
        if(not passThis):
            ...
            inst_losses = self.get_model().forward_train(torch.stack(mix_instance_img),
                                                ...
                                                scores_instance=scores_mixed_corrected,
                                                return_feat=True,
