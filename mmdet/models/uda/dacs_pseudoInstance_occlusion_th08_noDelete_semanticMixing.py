...
@UDA.register_module()
class DACSPSOCC08NODELETESEMANTICMIXING(UDADecorator):

    def __init__(self, **cfg):
        super(DACSPSOCC08NODELETESEMANTICMIXING, self).__init__(**cfg)
        self.local_iter = 0
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, # daformer args
        ...
        mix_masks = get_class_masks(ps_target_seg)
        #breakpoint()
        ...
        for i in range(batch_size):
            ...
            mixed_img[i], mixed_lbl[i] = strong_transform(strong_parameters, data=torch.stack((target_img[i], img[i] )), target=torch.stack((pseudo_label[i], gt_semantic_seg[i][0])))
            #_, pseudo_weight[i] = strong_transform(strong_parameters, target=torch.stack((gt_pixel_weight[i], pseudo_weight[i])))
            _, pseudo_weight[i] = strong_transform(strong_parameters, target=torch.stack((pseudo_weight[i], gt_pixel_weight[i])))
        mixed_img = torch.cat(mixed_img)
        mixed_lbl = torch.cat(mixed_lbl)
        #from torchvision.utils import save_image
        #save_image((target_img[0]+2.)/2., './targ.png')
        #save_image((img[0]+2.)/2., './src.png')
        #save_image((mixed_img[0]+2.)/2., './mix.png')
        #breakpoint()
        ...
        if(visual and not passThis):
            ...
            save_image((img[0]+2.)/2., repo+'save0.png')
            save_image((img[1]+2.)/2., repo+'save1.png')
            for j in range(batch_size):  
                save_image((img_pastes[j]+2.)/2., repo+'cars_'+str(self.iter)+'.png')
            #breakpoint()
