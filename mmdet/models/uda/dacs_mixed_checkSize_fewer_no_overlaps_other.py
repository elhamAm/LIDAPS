...
@UDA.register_module()
class DACSMixedCheckSizeFewerNoOverlaps(UDADecorator):

    def __init__(self, **cfg):
        super(DACSMixedCheckSizeFewerNoOverlaps, self).__init__(**cfg)
        self.local_iter = 0
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, # daformer args
        ...
        if(apply):
            ...
            if(True in list(conds0) or True in list(conds1)):
                ...
                gt_boxes_inst_cars=torch.cat([t0,t1])
                ...
                #breakpoint()
                labels_stacked_0 = torch.zeros(img.shape[2], img.shape[3])
                labels_stacked_1 = torch.zeros(img.shape[2], img.shape[3])
                #print(gt_masks_torch1.shape)
                allMasks0=torch.tensor(gt_masks[0].masks).to('cuda')
                allMasks1=torch.tensor(gt_masks[1].masks).to('cuda')
                for maski in range(allMasks0.shape[0]):
                    labels_stacked_0[allMasks0[maski]==1]+=1
                for maski in range(allMasks1.shape[0]):
                    labels_stacked_1[allMasks1[maski]==1]+=1

                #labels_stacked_0[labels_stacked_0>1]
                print('the shape: ', gt_masks_torch1.shape[0])
                print(labels_stacked_0[labels_stacked_0>1])
                print('##############################################')
                print('##############################################')
                #breakpoint()
                for t in range(3):
                    ...
                    rolled_masks0 = torch.roll(gt_masks_torch, (moveX,moveY),(1,2))
                    rolled_masks1 = torch.roll(gt_masks_torch, (moveX,moveY),(1,2))
                    gt_boxes_inst0_=copy.deepcopy(gt_boxes_inst_cars)
                    gt_boxes_inst1_=copy.deepcopy(gt_boxes_inst_cars)
                    #t_ = copy.deepcopy(t)


                    #breakpoint()
                    for imask in range(rolled_masks0.shape[0]):
                        #breakpoint()
                        labels_stacked_1[rolled_masks1[imask]==1]+=1
                        breakpoint()
                        rolled_masks1[imask][labels_stacked_1>1]=0

                        labels_stacked_0[rolled_masks0[imask]==1]+=1
                        rolled_masks0[imask][labels_stacked_0>1]=0

                        #breakpoint()
                        if(labels_stacked_1[labels_stacked_1>1].shape[0] > 0):#not empty
                            
                            #breakpoint()
                            minIndy=np.min(np.argwhere(rolled_masks1[imask].cpu().numpy()>0)[:, 1])
                            maxIndy=np.max(np.argwhere(rolled_masks1[imask].cpu().numpy()>0)[:, 1])

                            minIndx=np.min(np.argwhere(rolled_masks1[imask].cpu().numpy()>0)[:, 0])
                            maxIndx=np.max(np.argwhere(rolled_masks1[imask].cpu().numpy()>0)[:, 0])

                            #t0_=
                            #breakpoint()
                            gt_boxes_inst1_[imask][0]=minIndy
                            gt_boxes_inst1_[imask][2]=maxIndy
                            gt_boxes_inst1_[imask][1]=minIndx
                            gt_boxes_inst1_[imask][3]=maxIndx
                        else:

                            gt_boxes_inst1_[imask][0]+=moveY
                            gt_boxes_inst1_[imask][2]+=moveY
                            gt_boxes_inst1_[imask][1]+=moveX
                            gt_boxes_inst1_[imask][3]+=moveX
                        labels_stacked_1[labels_stacked_1>1]=0

                        if(labels_stacked_0[labels_stacked_0>1].shape[0] > 0):#not empty
                            
                            #breakpoint()
                            minIndy=np.min(np.argwhere(rolled_masks0[imask].cpu().numpy()>0)[:, 1])
                            maxIndy=np.max(np.argwhere(rolled_masks0[imask].cpu().numpy()>0)[:, 1])

                            minIndx=np.min(np.argwhere(rolled_masks0[imask].cpu().numpy()>0)[:, 0])
                            maxIndx=np.max(np.argwhere(rolled_masks0[imask].cpu().numpy()>0)[:, 0])

                            #t0_=
                            gt_boxes_inst0_[imask][0]=minIndy
                            gt_boxes_inst0_[imask][2]=maxIndy
                            gt_boxes_inst0_[imask][1]=minIndx
                            gt_boxes_inst0_[imask][3]=maxIndx
                        else:

                            gt_boxes_inst0_[imask][0]+=moveY
                            gt_boxes_inst0_[imask][2]+=moveY
                            gt_boxes_inst0_[imask][1]+=moveX
                            gt_boxes_inst0_[imask][3]+=moveX

                        labels_stacked_0[labels_stacked_0>1]=0
                    ...
                    #    t1_[:,3]+=moveX
                    #breakpoint()
                    if (gt_boxes_inst0_.shape[0] > 0 ):#and t0_.shape[1] > 0):
                        gt_bboxes_inst[0]=torch.cat([gt_bboxes_inst[0],gt_boxes_inst0_])
                        gt_bboxes_inst[1]=torch.cat([gt_bboxes_inst[1],gt_boxes_inst1_])
                    ...
                    #if (t0.shape[0] > 0 and t0.shape[1] > 0):
                    twos=torch.tensor(np.array([2]*(gt_boxes_inst0_.shape[0]))).to('cuda')
                    ...
                    all_masks0=torch.cat([torch.tensor(gt_masks_inst[0].masks).to('cuda'),rolled_masks0])
                    gt_masks_inst[0]=BitmapMasks(all_masks0.cpu().numpy(), rolled_masks0.shape[1], rolled_masks0.shape[2])

                    #if(t1.shape[0] > 0):
                    #print('mask shape 1: ', gt_masks_inst[1].masks.shape)
                    all_masks1=torch.cat([torch.tensor(gt_masks_inst[1].masks).to('cuda'),rolled_masks1])
                    gt_masks_inst[1]=BitmapMasks(all_masks1.cpu().numpy(), rolled_masks1.shape[1], rolled_masks1.shape[2])
                    ...
                    for m in range(rolled_masks0.shape[0]):
                        if(m < t0.shape[0]):
                            img_pastes[0] = img_temp[0] * rolled_masks0[m] + img_pastes[0] * (1-rolled_masks0[m])
                            img_pastes[1] = img_temp[0] * rolled_masks1[m] + img_pastes[1] * (1-rolled_masks1[m])
                        else:
                            img_pastes[0] = img_temp[1] * rolled_masks0[m] + img_pastes[0] * (1-rolled_masks0[m])
                            img_pastes[1] = img_temp[1] * rolled_masks1[m] + img_pastes[1] * (1-rolled_masks1[m])
                visual=True
                #breakpoint()
                if(visual):
