...
@UDA.register_module()
class DACSPSOCCPARAMNODELETE(UDADecorator):

    def __init__(self, **cfg):
        super(DACSPSOCCPARAMNODELETE, self).__init__(**cfg)
        self.local_iter = 0
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, # daformer args
        ...
        for j in range(2):
            ...
            num_classes=8
            mask_score_th=self.train_cfg['filterThres']#0.8
            #breakpoint()
        ...
        if(visual and not passThis):
            ...
            save_image((img[0]+2.)/2., repo+'source0'+'_'+str(self.iter)+'.png')
            save_image((img[1]+2.)/2., repo+'source1'+'_'+str(self.iter)+'.png')

            save_image((target_img[0]+2.)/2., repo+'target0'+'_'+str(self.iter)+'.png')
            save_image((target_img[1]+2.)/2., repo+'target1'+'_'+str(self.iter)+'.png')
            ...
            for q in range(masks.shape[0]):
                ...
                img_pastes[j]=(1-masks[q].unsqueeze(0).repeat(3,1,1))*img_pastes[j]
                #breakpoint()
                img_masksInstTarget[j]=(masks[q].unsqueeze(0).repeat(3,1,1))+img_masksInstTarget[j]
                #breakpoint()
            save_image((img_pastes[j]+2)/2., repo+'carsMasks_'+str(self.iter)+'.png')
            save_image((img_masksInstTarget[j]), repo+'maskInstTargets_'+str(self.iter)+'.png')
            self.iter+=1
        #breakpoint()
        #print('here')
