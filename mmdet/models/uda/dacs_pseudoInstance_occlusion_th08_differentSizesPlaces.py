...
@UDA.register_module()
class DACSPSOCC08SIZEPLACE(UDADecorator):

    def __init__(self, **cfg):
        super(DACSPSOCC08SIZEPLACE, self).__init__(**cfg)
        self.local_iter = 0
                ...
                for c in classes:
                    ...
                    for m in range(len(masks_c)):
                        ...
                        if mask_score >= mask_score_th and np.unique(masks_c[m]).shape[0]>1:
                            ...
                            imgTemp = Fun.interpolate(target_img[j].to('cuda').unsqueeze(0), [new_w_h, new_w_h], mode='bilinear', align_corners=True)
