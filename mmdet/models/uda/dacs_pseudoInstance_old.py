...
@UDA.register_module()
class DACSPSOLD(UDADecorator):

    def __init__(self, **cfg):
        super(DACSPSOLD, self).__init__(**cfg)
        self.local_iter = 0
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, # daformer args
        ...
        for j in range(2):
            ...
            num=0
            passThis =False
            while(boxes[c].shape[0] < 0 or not(True in list(boxes[c][:,-1]>mask_score_th))):
                c=random.randint(0, 7)
                #print(c)
                num+=1
                if(num>15):
            ...
            if(not passThis):
                ...
                for m in range(len(masks_c)):
                    ...
                    if mask_score >= mask_score_th:
                        ...
                        new_boxes.append(torch.tensor(boxes_c[m,:-1]))
