...
@UDA.register_module()
class DACSCLIP(UDADecorator):

    def __init__(self, **cfg):
        super(DACSCLIP, self).__init__(**cfg)
        self.local_iter = 0
    ...
    def calc_feat_dist(self, img, gt, feat=None, feat_imnet=None):
        #breakpoint()
        assert self.enable_fdist
        #with torch.no_grad():
        #breakpoint()
        #self.get_imnet_model().eval()
        #feat_imnet, _, feat_clip = self.get_imnet_model().extract_feat(img)
        #breakpoint()
        feat_imnet = feat_imnet.detach()#[f.detach() for f in feat_imnet]
        lay = -1
        #breakpoint()
        if self.fdist_classes is not None:
            ...
            feat_dist = self.masked_feat_dist(feat[lay], feat_imnet[:,:512,:,:], fdist_mask)#feat_imnet[lay], fdist_mask)
            self.debug_fdist_mask = fdist_mask
            self.debug_gt_rescale = gt_rescaled
        else:
            feat_dist = self.masked_feat_dist(feat[lay], feat_imnet[:,:512,:,:])#feat_imnet[lay])
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, # daformer args
        ...
        clip_feat = clean_losses.pop('features_clip')
        #breakpoint()
        ...
        if self.enable_fdist:
            #breakpoint()
            feat_loss, feat_log = self.calc_feat_dist(img, gt_semantic_seg, src_feat, clip_feat)
