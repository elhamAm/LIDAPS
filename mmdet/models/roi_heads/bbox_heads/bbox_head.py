...
class BBoxHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 loss_cls_pseudo=dict(type='CrossEntropyLoss', use_sigmoid=False, loss_weight=0.0),
                 loss_bbox_pseudo=dict(type='SmoothL1Loss', beta=1.0, loss_weight=0.0),
                 init_cfg=None):
        ...
        self.loss_cls_pseudo = build_loss(loss_cls_pseudo)
        self.loss_bbox_pseudo = build_loss(loss_bbox_pseudo)
    ...
    def get_targets(self,
        ...
        if gt_box_domain_indicator_list is not None:
            ...
            pos_assigned_gt_inds_list = [None for _ in range(num_imgs)]
    ...
    def loss(self,
        ...
        #(Pdb) pos_bbox_pred[torch.isnan(bbox_targets[pos_inds.type(torch.bool)])[:,0]==True]
        #tensor([[-4.9706e-06,  1.0006e-05, -8.7507e-06,  9.9872e-06]], device='cuda:0',
        #grad_fn=<IndexBackward0>)
        #(Pdb) bbox_targets[pos_inds.type(torch.bool)][torch.isnan(bbox_targets[pos_inds.type(torch.bool)])[:,0]==True]
        #tensor([[nan, 0., nan, 0.]], device='cuda:0')
        losses = dict()
        if cls_score is not None:
            ...
            if cls_score.numel() > 0:
                if not use_instance_pseduo_losses:
                    ...
                    loss_cls_ = self.loss_cls_pseudo(
                        cls_score,
                        labels,
                        label_weights,
                        avg_factor=avg_factor,
        ...
        #breakpoint()
        #print('bbox_pred is not None: ', bbox_pred is not None)
        if bbox_pred is not None:
            ...
            #sprint('pos_inds.any(): ',pos_inds.any())
            if pos_inds.any():
                #breakpoint()
                ...
                if self.reg_decoded_bbox:
                    ...
                    #breakpoint()
                    bbox_pred = self.bbox_coder.decode(rois[:, 1:], bbox_pred)#[pos_inds.type(torch.bool)]
                ...
                if self.reg_class_agnostic:
                    ...
                    #(Pdb) pos_bbox_pred.shape
                    #torch.Size([145, 4])
                    pos_bbox_pred = bbox_pred.view(bbox_pred.size(0), -1, 4)[pos_inds.type(torch.bool), labels[pos_inds.type(torch.bool)]]

                #breakpoint()
                #print('length of pos inds: ', torch.sum(pos_inds == True))
                #print('pred bbox: ', 'min: ', torch.min(pos_bbox_pred), 'max: ', torch.max(pos_bbox_pred))
                #print('target bbox: ', 'min: ', torch.min(bbox_targets[pos_inds.type(torch.bool)]), 'max: ', torch.max(bbox_targets[pos_inds.type(torch.bool)]))
                #print('first element: target and pred: ', bbox_targets[pos_inds.type(torch.bool)][0], pos_bbox_pred[0])
                #print('bbox weights: ', 'min: ', torch.min(bbox_weights[pos_inds.type(torch.bool)]), 'max: ', torch.max(bbox_weights[pos_inds.type(torch.bool)]))
                #print('in loss bbox head')
                
                #bbox_targets[pos_inds.type(torch.bool)][torch.isnan(bbox_targets[pos_inds.type(torch.bool)])[:,0]==True]=t
                #pos_bbox_pred[torch.isnan(bbox_targets[pos_inds.type(torch.bool)])[:,0]==True]=t
                #bbox_weights[pos_inds.type(torch.bool)][torch.isnan(bbox_targets[pos_inds.type(torch.bool)])[:,0]==True]=t
                if not use_instance_pseduo_losses:
                    #it enters here
                    t=torch.tensor([0.,0.,0.,0.]).to('cuda')
                    inds=torch.all(~(torch.isnan(bbox_targets[pos_inds.type(torch.bool)])), dim=1)
                    #breakpoint()
                    #if(bbox_targets[pos_inds.type(torch.bool)].shape[0]-inds.shape[0] > 0):
                    #breakpoint()
                    #print(bbox_targets[pos_inds.type(torch.bool)].shape[0]-inds.shape[0])
                    num=bbox_targets.size(0)
                    bbox_targets=bbox_targets[pos_inds.type(torch.bool)][inds]
                    bbox_weights=bbox_weights[pos_inds.type(torch.bool)][inds]
                    pos_bbox_pred=pos_bbox_pred[inds]
                    #breakpoint()
                    ...
                    losses['loss_bbox'] = self.loss_bbox_pseudo(
                        pos_bbox_pred,
                        bbox_targets[pos_inds.type(torch.bool)],
                        bbox_weights[pos_inds.type(torch.bool)],
            ...
            #print('roi loss bbox: ',losses['loss_bbox'])
            #breakpoint()
        return losses,pos_bbox_pred_
    ...
    def get_bboxes(self,
        ...
        #breakpoint()
        #bboxes.size() torch.Size([1000, 32])
        if rescale and bboxes.size(0) > 0: # False
            #breakpoint()
