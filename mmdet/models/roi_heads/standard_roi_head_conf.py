...
class StandardRoIHeadCONF(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def forward_train(self,
        ...
        if(scores_instance is None):
            ...
            for j in range(len(gt_bboxes)):
                #breakpoint()
                scores_instance.append(torch.ones(gt_bboxes[j].shape[0]))

        if self.with_bbox:
            #breakpoint()
            #scale factor is array([1., 1., 1., 1.]
            #breakpoint()
            #print('in standard roi head bbox forward train')
            #breakpoint()
            bbox_results, bb = self._bbox_forward_train(img,x,
                                                    sampling_results,
                                                    gt_bboxes,
                                                    gt_labels,
                                                    img_metas,
            ...
            #print('bbox_results shape: ', bbox_results.shape) 
        #breakpoint()
        ...
        if self.with_mask:
            ...
            #print('bbox_results shape: ', mask_results.keys)
            #breakpoint()

        return losses, bb#, bbox_results
    ...
    def _bbox_forward_train(
        ...
        det_bbox, det_label = self.bbox_head.get_bboxes(
                                                                 ...
                                                                 1,
                                                                 rescale=False,
                                                                 cfg=ConfigDict({'score_thr': 0.05, 'nms': {'type': 'nms', 'iou_threshold': 0.4}, 'max_per_img': 100, 'mask_thr_binary': 0.5}),
                                                                     )
        #breakpoint()
        ...
        #breakpoint()
        #print('in standard roi head self.bbox_head.loss')
        #breakpoint()
        #breakpoint()
        loss_bbox,bb2 = self.bbox_head.loss(bbox_results['cls_score'],
        ...
        #breakpoint()
        return bbox_results, det_bbox #bb2
    ...
    def _mask_forward_train(self, x, sampling_results, bbox_feats, gt_masks, img_metas,
                            ...
                            use_instance_pseduo_losses=False,scores_instance=None,
                            ):
        ...
        j =0
        for res in sampling_results:
            mask_weights.append(scores_instance[j][res.pos_assigned_gt_inds].to('cuda'))
            j+=1
        #breakpoint()
        mask_weights=torch.cat(mask_weights)
        mask_weights=mask_weights.repeat(mask_targets.shape[1]*mask_targets.shape[2],1).permute(1,0).reshape(mask_targets.shape[0],mask_targets.shape[1],mask_targets.shape[2]).to('cuda')
        #breakpoint()
        # -------------------------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------------------------
            ...
            loss_mask = self.mask_head.loss(mask_results['mask_pred'], mask_targets, pos_labels,mask_weights=mask_weights )#Elham
    ...
    def _mask_forward(self, x, rois=None, pos_inds=None, bbox_feats=None):
        ...
        if rois is not None: # True
            mask_feats = self.mask_roi_extractor(x[:self.mask_roi_extractor.num_inputs], rois)#here
            if self.with_shared_head:
        ...
        #breakpoint()
        mask_pred = self.mask_head(mask_feats)#here
        #breakpoint()
    ...
    def simple_test(self,
        ...
        if not self.with_mask:
            ...
            #breakpoint()
            segm_results, segm_results_feat = self.simple_test_mask( x, img_metas, det_bboxes, det_labels, rescale=rescale, isTrain=isTrain,takeFeat=takeFeat)
            
            #breakpoint()
            #breakpoint()
            return list(zip(bbox_results, segm_results)), segm_results_feat
