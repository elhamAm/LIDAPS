...
class StandardRoIHeadClipMask(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            ...
            #breakpoint()
            bbox_results, bb, rois, bbox_feat_gt = self._bbox_forward_train(img,x,
                                                    sampling_results,
                                                    gt_bboxes,
                                                    gt_labels,
                                                    img_metas,
        ...
        return losses, bb, rois, bbox_feat_gt#, bbox_results
    ...
    def _bbox_forward_train(
        ...
        bbox_results = self._bbox_forward(x, rois)
        #breakpoint()
        t0=torch.zeros(gt_bboxes[0].shape[0])
        t0=t0.to('cuda')
        new_gt_bb0=torch.cat((t0.unsqueeze(1),gt_bboxes[0]),dim=1)

        t1=torch.ones(gt_bboxes[1].shape[0])
        t1=t1.to('cuda')
        new_gt_bb1=torch.cat((t1.unsqueeze(1),gt_bboxes[1]),dim=1)
        #breakpoint()
        new_gt_bb=torch.cat([new_gt_bb0,new_gt_bb1], dim=0)
        
        #bbox_feats_gt = self.bbox_roi_extractor(x[:4], new_gt_bb)
        mask_feats_gt = self.mask_roi_extractor(x[:self.mask_roi_extractor.num_inputs], new_gt_bb)
        #breakpoint()
        ...
        #breakpoint()
        return bbox_results, det_bbox, rois, mask_feats_gt #bb2
