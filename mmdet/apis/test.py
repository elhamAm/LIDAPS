...
def single_gpu_test_uda_for_visual_debug(model,
    ...
    prog_bar = mmcv.ProgressBar(len(dataset))
    #breakpoint()
    for im, data in enumerate(data_loader):
        if True:
            with torch.no_grad():
                if debug or dataset_name == 'mapillary':
                    rescale = False
            ...
            #out_dict['masks_feat'] = results[i]['ins_results_feat'][0]#[0][1]
            #breakpoint()
            
            print('boxes: ', len(results[i]['ins_results'][0][0]))
            print('boxes: ', len(results[i]['ins_results'][0][0][0].shape))
            print('masks: ', len(results[i]['ins_results'][0][1]))
            # semantic seg
            ...
            #print(data)
            #breakpoint()
            save_image(data['img'][0][0], name)  
            instances, ins_seg, pan_seg_thing_classes, ins_seg_cars, ins_cars, ins_seg_mask_cars_feat, boxes_cars = \
                get_cityscapes_instance_format_for_maskrcnn(
                    out_dict['boxes'],
                    out_dict['masks'],
                    #masks_feat=out_dict['masks_feat'],
            ...
            #np.save('maskFeats/car_instances_'+str(im)+'.npy', ins_cars)
            #breakpoint()
            #np.save('maskFeats/car_instances_feat_'+str(im)+'.npy', ins_seg_mask_cars_feat)
