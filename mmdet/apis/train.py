...
def train_detector(model, dataset, cfg, distributed=False, validate=False, timestamp=None, meta=None,):
    ...
    # Build the runner
    #breakpoint()
    #cfg.work_dir='PATH_TO_FILL/edaps_experiments/exp-00827/work_dirs/local-exp00827/231121_1041_syn2cs_dacs_rcs001_cpl_maskrcnn_mitb5_poly10warm_s0_866ae'#Elham
    #cfg.resume_from = find_latest_checkpoint(cfg.work_dir)#Elham
    ...
    #breakpoint()
    #checkpoint_path = cfg.checkpoint_path#Elham
    #model.load(checkpoint_path)#Elham
    #optimizer.load(checkpoint_path)#Elham

    resume_from = None
    
    #resume_elham=True
    #breakpoint()
    if('resumeElham' in cfg.keys()):
        #cfg.work_dir='PATH_TO_FILL/edaps_experiments/exp-00827/work_dirs/local-exp00827/231121_1041_syn2cs_dacs_rcs001_cpl_maskrcnn_mitb5_poly10warm_s0_866ae'#Elham
        #cfg.resume_from = find_latest_checkpoint(cfg.work_dir)#Elham
        cfg.resume_from = cfg['resumeElham']#find_latest_checkpoint(cfg['resumeElham'])
        #breakpoint()
        #breakpoint()#Elham
        runner.resume(cfg.resume_from)#Elham
        #breakpoint()
