    ...
    def evaluate(self, results, metric='mIoU', logger=None, imgfile_prefix=None, efficient_test=False,
        ...
        if len(metrics) > 0:
            if eval_type == 'daformer':
                eval_results.update(super(CityscapesDataset,self).evaluate(results, metrics, logger, efficient_test, evalScale))
                ...
                eval_results.update(
                    super(CityscapesDataset, self).evaluate_panoptic_for_maskformer(
                ...
                breakpoint()
                eval_results.update(
                    super(CityscapesDataset, self).evaluate_instance_for_maskrcnn(
                        results, cuda, panop_eval_temp_folder, dataset_name, gt_dir, debug, num_samples_debug,
                ...
                eval_results.update(
                    super(CityscapesDataset, self).evaluate_panoptic_for_maskrcnn(
                        results, cuda, panop_eval_temp_folder, dataset_name, gt_dir, debug, num_samples_debug,
                        gt_dir_panop, logger, post_proccess_params, visuals_pan_eval, out_dir, metric, evalScale,evaluate_from_saved_numpy_predictions
                                                                                )
                )
            else:
