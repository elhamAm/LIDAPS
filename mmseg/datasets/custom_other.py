...
class CustomDataset(Dataset):
    ...
    def get_debug_img_list(self, dataset_name):
        if dataset_name == 'cityscapes': # or dataset_name == 'cityscapesFoggy':
    ...
    def get_gt_panoptic_labels(self, device, logger, debug,
        ...
        for img_info in self.img_infos:
            ...
            if dataset_name == 'cityscapes' and evalScale == '1024x512':
                gt_panoptic_seg = gt_panoptic_seg.resize((1024, 512), Image.NEAREST)  # TODO
            ...
            # elif dataset_name == 'cityscapesFoggy' and evalScale is None:
            #     pass
    ...
    def evaluate_panoptic_for_maskrcnn_v2( self, results, device=None, panop_eval_temp_folder=None,
        ...
        if dataset_name == 'cityscapes':
            ...
            stuff_area = 2048
            input_image_size = (1024, 768) # new_img_shapes # (1024, 768) # Not in use
    ...
    def evaluate_panoptic_for_maskrcnn( self, results, device=None, panop_eval_temp_folder=None,
        ...
        # results already has all the prediction results on cityscapesFoggy val images dataset_name='cityscapesFoggy'
        # this function just access the ground truth labels which are same for both cityscapes and cityscapesFoggy
        # So, we need to simply set the dataset_name= 'cityscapes'
        ...
        eval_folder['semantic'] = os.path.join(panop_eval_temp_folder, 'semantic')
        eval_folder['panoptic'] = os.path.join(panop_eval_temp_folder, 'panoptic')
        os.makedirs(eval_folder['visuals'], exist_ok=True)
        logger.info('****************************')
        logger.info('Folder created.')
        logger.info(eval_folder['visuals'])
        logger.info('****************************')

        # setting JSON files
        image_filename_list = []
        visual_img_filename_list = []
        for i in range(len(self.img_infos)):
            image_filename_list.append(self.img_infos[i]['ann']['seg_map'].split('.')[0])
            visual_img_filename_list.append(self.img_infos[i]['filename'])
        ...
        try:
            for i in range(num_samples):
                if dataset_name == 'cityscapes':
                    image_filename = image_filename_list[i].split('/')[1]
                    visual_img_filename = visual_img_filename_list[i]
                elif dataset_name == 'mapillary':
                    image_filename = image_filename_list[i]
                else:
                    raise NotImplementedError(f'Implementation not found for dataset: {dataset_name}')
                ...
                if visuals_pan_eval:
                    ...
                    save_predictions(dataset_name, image_filename, TrgSemGT, TrgSemPd, TrgPanPd, outdir,
                                     debug, self.img_dir, self.ann_dir, resize_with_pad, visual_img_filename)
    ...
    def evaluate_panoptic(self, results, device=None, panop_eval_temp_folder=None,
        ...
        CENTER_THRESHOLD = post_proccess_params['center_threshold']
