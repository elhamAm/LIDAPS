...
class CustomDataset(Dataset):
    ...
    def __init__(self,
        ...
        #self.data_root='PATH_TO_FILL/edaps/data/cityscapes/'
        #breakpoint()
    ...
    def load_annotations_panoptic(self, ann_dir):
        ...
        for ano in dataset['annotations']:
            ...
            elif ('cityscapes' in self.data_root) and ('cityscapesFoggy' not in self.data_root):
                ano_fname = ano['image_id']
                str1 = ano_fname.split('_')[0] + '/' + ano_fname
                ano_fname = str1 + '_leftImg8bit.png'
                seg_fname = str1 + self.seg_map_suffix
            elif 'cityscapesFoggy' in self.data_root:
                ano_fname = ano['image_id']
                str1 = ano_fname.split('_')[0] + '/' + ano_fname
                ano_fname = str1 + self.img_suffix[0]
                seg_fname = str1 + self.seg_map_suffix
    ...
    def evaluate_panoptic_for_maskrcnn( self, results, device=None, panop_eval_temp_folder=None,
        ...
        '''
        #breakpoint()
        if 'cityscapesFoggy' in dataset_name:
            dataset_name = 'cityscapes'
        ...
        print('upsnet_mask_pruning: ', upsnet_mask_pruning)
        ...
        if evaluate_from_saved_numpy_predictions:
            ...
            #breakpoint()
            numpys_path = panop_eval_temp_folder+'/results_numpys'#+os.path.join(strs[0], strs[1], strs[2], strs[3], strs[4], 'results_numpys')
            #numpys_path=#'./edaps_experiments/exp-01002/work_dirs/local-exp01002/230808_1221_syn2cs_dacs_rcs001_cpl_maskrcnn_mitb5_poly10warm_s0_ce05b/results_numpys'
        ...
        samples_with_merging=[]

        if(evaluate_from_saved_numpy_predictions):
            if (not os.path.isdir('merges/gt')):
                #os.mkdir('merges')
                #os.mkdir('nomerges')
                os.mkdir('merges/gt')
                os.mkdir('merges/masks_pred')
                os.mkdir('merges/orig')
                os.mkdir('nomerges/gt')
                os.mkdir('nomerges/masks_pred')
                os.mkdir('nomerges/orig')
                os.mkdir('crops')
            else:
                shutil.rmtree('merges/gt')
                shutil.rmtree('merges/masks_pred')
                shutil.rmtree('merges/orig')
                shutil.rmtree('nomerges/gt')
                shutil.rmtree('nomerges/masks_pred')
                shutil.rmtree('nomerges/orig')
                os.mkdir('merges/gt')
                os.mkdir('merges/masks_pred')
                os.mkdir('merges/orig')
                os.mkdir('nomerges/gt')
        ...
        try:
            for i in range(num_samples):
                ...
                if evaluate_from_saved_numpy_predictions:
                    image_filename_res = image_filename.replace("gtFine_panoptic","leftImg8bit")
                    numpy_file_path = os.path.join(numpys_path, f'{image_filename_res}.png.npy')
                    #breakpoint()
                    ...
                    # instance seg
                    #breakpoint()
                    if('ins_results' in results[i].keys()):
                ...
                gt_labels['instance'] = gt_panoptic_labels[i][1]
                ...
                #breakpoint()
                #len(results[0][0]['ins_results'][0][0])
                ...
                if 'mAP' in metric:
                    if upsnet_mask_pruning:
                        ...
                        instances, ins_seg, pan_seg_thing, ins_seg_cars, ins_cars,_, boxes_cars = get_cityscapes_instance_format_for_maskrcnn(
                                                                            out_dict['boxes'],
                                                                            out_dict['masks'],
                                                                            pred_shape=pred_shape,
                                                                            mask_score_th=mask_th,
                ...
                if(evaluate_from_saved_numpy_predictions):
                    ...
                    gt_seg = np.zeros(pred_shape).astype(int)
                    #if(image_filename_res=='frankfurt_000000_001016_leftImg8bit'):
                    #    breakpoint()

                    mergedMasks=[]
                    print('starting..')
                    place=image_filename.split('_')[0]
                    #if('9688' in image_filename):
                    #    breakpoint()

                    root="data/cityscapes/leftImg8bit/val/"+place+'/'
                    for j in range(len(ins_cars)):
                        overlap=0
                        pred = ins_cars[j]
                        for sa in range(len(gt_panoptic_labels[i][1])):
                            if(gt_panoptic_labels[i][2][sa]==2):
                                gt = gt_panoptic_labels[i][1].masks[sa]
                                if(j == 0):
                                    gt_seg[gt==1] = color
                                    color+=30
                                    
                                #breakpoint()
                                common = torch.sum(torch.mul(torch.tensor(gt),torch.tensor(pred)))
                                if(common > 20):
                                    overlap+=1
                        if(overlap > 1):
                            carMerging=True
                            mergedMasks.append(j)
                        #print('image_filename: ', image_filename_list[i])
                        from PIL import Image
                        #breakpoint()
                        im = Image.open(root+image_filename_res+'.png')
                        #print(boxes_cars[j])
                        #(left, top, right, bottom)
                        im1 = im.crop((boxes_cars[j][0], boxes_cars[j][1], boxes_cars[j][2], boxes_cars[j][3]))
                        im1.save('./crops/im_'+str(i)+'_'+'seg'+str(j)+'.png')
                    if(carMerging):
                        #print(image_filename_res)
                        
                        name='./merges/masks_pred/'+image_filename_res+'.png'
                        im=torch.stack( [torch.tensor(ins_seg_cars),  torch.tensor(ins_seg_cars),  torch.tensor(ins_seg_cars)])
                        save_image(im.float()*0.001, name)

                        
                        name='./merges/gt/'+image_filename_res+'.png'
                        im=torch.stack( [torch.tensor(gt_seg),  torch.tensor(gt_seg),  torch.tensor(gt_seg)])
                        save_image(im.float()*0.001, name)
                        
                        shutil.copyfile(root+image_filename_res+'.png', './merges/orig/'+image_filename+'.png')
                    else:
                        name='./nomerges/masks_pred/'+image_filename_res+'.png'
                        im=torch.stack( [torch.tensor(ins_seg_cars),  torch.tensor(ins_seg_cars),  torch.tensor(ins_seg_cars)])
                        save_image(im.float()*0.001, name)

                        name='./nomerges/gt/'+image_filename_res+'.png'
                        im=torch.stack( [torch.tensor(gt_seg),  torch.tensor(gt_seg),  torch.tensor(gt_seg)])
                        save_image(im.float()*0.001, name)

                        shutil.copyfile(root+image_filename_res+'.png', './nomerges/orig/'+image_filename+'.png')

                        
                    #breakpoint()
                    #print('carMerging: ', carMerging)
                    
                    samples_with_merging.append({'name':root+image_filename_res+'.png', 'mergedMasks':mergedMasks})
                    print('samples_with_merging: ', {'name':root+image_filename_res+'.png', 'mergedMasks':mergedMasks})
                ...
                visuals_pan_eval=False#True
                #breakpoint()
                out_dict['masks']

                TrgInstPd=ins_seg.int()#torch.zeros(gt_labels['semantic'].shape)
                #breakpoint()
                TrgInstGt=torch.zeros(gt_labels['semantic'].shape)

                #########pred
                id=1
                for i in range(gt_labels['instance'].masks.shape[0]):
                    mask=gt_labels['instance'].masks[i]
                    TrgInstGt+=torch.tensor(mask*id)
                    id+=1
                TrgInstGt=TrgInstGt.int()
                #breakpoint()
                #id=1
                #for maskList in out_dict['masks']:
                #    for mask in maskList:
                #        TrgInstPd+=torch.tensor(mask*id)
                if visuals_pan_eval:
                    ...
                    save_predictions(dataset_name, image_filename, TrgSemGT, TrgSemPd, TrgPanPd, TrgInstPd, TrgInstGt, outdir, debug, self.img_dir, self.ann_dir, resize_with_pad)

            #import numpy as np
            samples_with_merging = np.asarray(samples_with_merging)
            # save to npy file
            print('samples_with_merging: ',samples_with_merging)
            np.save('samples_with_merging.npy', samples_with_merging)
            ...
            if 'mPQ' in metric:
                ...
                mPQ_16 = panoptic_results['All']['pq_16']
            ...
            if 'mIoU' in metric:
                if self.best_miou < mIoU:
                    ...
                    logger.info('*** BEST mIoU for 16 classes: {} ***'.format(self.best_miou*19/16))
            if 'mPQ' in metric:
                logger.info('*** Corresponding PQ: {} ***'.format(mPQ))
                logger.info('*** Corresponding PQ_16: {} ***'.format(mPQ_16))
