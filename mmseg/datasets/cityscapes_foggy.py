...
class CityscapesFoggyDataset(CustomDataset):
    """Cityscapes dataset.
    ...
    def __init__(self, **kwargs):
        print()
        super(CityscapesFoggyDataset, self).__init__(
    ...
    def evaluate(self, results, metric='mIoU', logger=None, imgfile_prefix=None, efficient_test=False,
        ...
        if len(metrics) > 0:
            if eval_type == 'daformer':
                eval_results.update(super(CityscapesFoggyDataset,self).evaluate(results, metrics, logger, efficient_test, evalScale))
            elif eval_type == 'panop_deeplab':
                eval_results.update(
                    super(CityscapesFoggyDataset,self).evaluate_panoptic(
                                                                        results, cuda, panop_eval_temp_folder,
                                                                        dataset_name, gt_dir, debug,
                                                                        num_samples_debug, gt_dir_panop, logger,
                                                                        post_proccess_params, visuals_pan_eval,
                                                                        evalScale, metric
                                                                    )
                                    )
            elif eval_type == 'maskformer': # eval mask based mIoU, mPQ, mAP
                eval_results.update(
                    super(CityscapesFoggyDataset, self).evaluate_panoptic_for_maskformer(
                ...
                eval_results.update(
                    super(CityscapesFoggyDataset, self).evaluate_instance_for_maskrcnn(
                        results, cuda, panop_eval_temp_folder, dataset_name, gt_dir, debug, num_samples_debug,
                        gt_dir_panop, logger, post_proccess_params, visuals_pan_eval, out_dir, evalScale
                                                                                )
                )
            elif eval_type == 'maskrcnn_panoptic':
                eval_results.update(
                    super(CityscapesFoggyDataset, self).evaluate_panoptic_for_maskrcnn(
                        results, cuda, panop_eval_temp_folder, dataset_name, gt_dir, debug, num_samples_debug,
