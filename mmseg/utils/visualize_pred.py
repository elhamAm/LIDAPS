...
def prep_ins_for_vis_mine_one(inst_map, mode='GT', dataset_name='cityscapes', debug=False, blend_ratio=0.7, img=None, show_only_person=False, sem_id=None):
    stuff_id = 0
    #breakpoint()
    if dataset_name=='cityscapes':
        if debug:
    ...
    for i in range(num_colors):
        inst_map[inst_map == ids[i]] = i
        #inst_target_map1[inst_target_map1 == ids[i]] = i
        #inst_target_map2[inst_target_map2 == ids[i]] = i
        colormap[i, :] = random_color(rgb=True, maximum=255)
        if ids[i] == stuff_id:
            colormap[i, :] = np.array([0, 0, 0])  # np.array([255, 0, 0])
    #breakpoint()
    ...
    colored_label = colormap[inst_map]
    #colored_label2 = colormap[inst_map2]
    #colored_label3 = colormap[inst_target_map1]
    #colored_label4 = colormap[inst_target_map2]
    #breakpoint()
    if img is not None:
        colored_label = blend_ratio * torch.tensor(colored_label.transpose(2,0,1)) + (1 - blend_ratio) * img#transpose(1,2,0)
        #colored_label2 = blend_ratio * torch.tensor(colored_label2.transpose(2,0,1)) + (1 - blend_ratio) * img2#transpose(1,2,0)

        #colored_label3 = blend_ratio * torch.tensor(colored_label3.transpose(2,0,1)) + (1 - blend_ratio) * target#transpose(1,2,0)
        #colored_label4 = blend_ratio * torch.tensor(colored_label4.transpose(2,0,1)) + (1 - blend_ratio) * target#transpose(1,2,0)
    return colored_label#1, colored_label2, colored_label3, colored_label4
def prep_ins_for_vis_mine(inst_map1, inst_map2, inst_target_map1, inst_target_map2, mode='GT', dataset_name='cityscapes', debug=False, blend_ratio=0.7, img1=None, img2=None,target=None, show_only_person=False, sem_id=None):
    stuff_id = 0
    #breakpoint()
    if dataset_name=='cityscapes':
        if debug:
            ...
            IGNORE_TOP_VAL = 15 * 2
            IGNORE_BOTTOM_VAL = 90 * 2
        inst_map1[:IGNORE_TOP_VAL, :] = 255
        inst_map1[-IGNORE_BOTTOM_VAL:, :] = 255
        # if show_only_person:
        #     inst_map[sem_id != 11, :] = 0
    elif dataset_name == 'Mapillary':
        raise NotImplementedError('Implementation not found for Mapillary dataset --> mmseg/datasets/custom.py !!')
    ids = np.unique(inst_map1)
    num_colors = len(ids)
    colormap = np.zeros((num_colors, 3), dtype=np.uint8)
    # Maps inst_map to continuous value.
    for i in range(num_colors):
        inst_map1[inst_map1 == ids[i]] = i
        inst_map2[inst_map2 == ids[i]] = i
        inst_target_map1[inst_target_map1 == ids[i]] = i
        inst_target_map2[inst_target_map2 == ids[i]] = i
        colormap[i, :] = random_color(rgb=True, maximum=255)
        if ids[i] == stuff_id:
    ...
    colored_label1 = colormap[inst_map1]
    colored_label2 = colormap[inst_map2]
    colored_label3 = colormap[inst_target_map1]
    colored_label4 = colormap[inst_target_map2]
    #breakpoint()
    if img1 is not None:
        colored_label1 = blend_ratio * torch.tensor(colored_label1.transpose(2,0,1)) + (1 - blend_ratio) * img1#transpose(1,2,0)
        colored_label2 = blend_ratio * torch.tensor(colored_label2.transpose(2,0,1)) + (1 - blend_ratio) * img2#transpose(1,2,0)

        colored_label3 = blend_ratio * torch.tensor(colored_label3.transpose(2,0,1)) + (1 - blend_ratio) * target#transpose(1,2,0)
        colored_label4 = blend_ratio * torch.tensor(colored_label4.transpose(2,0,1)) + (1 - blend_ratio) * target#transpose(1,2,0)
    return colored_label1, colored_label2, colored_label3, colored_label4
...
def save_predictions(dataset_name, image_filename, TrgSemGT, TrgSemPd, TrgPanPd, TrgInstPd, TrgInstGT, outdir, debug, img_dir, ann_dir, resize_with_pad):
    #breakpoint()
    if dataset_name == 'cityscapes':
        vis_W = 1024
        vis_H = 512
    ...
    subplotimgV2(axs[0][2], vis_trg_img, 'TrgImg')
    #breakpoint()
    subplotimgV2(axs[0][0], prep_sem_for_vis(TrgSemGT, mode='GT', dataset_name=dataset_name, debug=debug, blend_ratio=0.7, img=None), 'TrgSemGT')
    subplotimgV2(axs[0][1], prep_sem_for_vis(TrgSemPd, mode='PD', dataset_name=dataset_name, debug=debug, blend_ratio=0.7, img=None), 'TrgSemPd')
    
    #(inst_map, mode='GT', dataset_name='cityscapes', debug=False, blend_ratio=0.7, img=None, show_only_person=False, sem_id=None)
    #breakpoint()
    subplotimgV2(axs[1][0], prep_ins_for_vis(TrgInstGT, mode='GT', dataset_name=dataset_name, debug=debug, blend_ratio=0.7, img=vis_trg_img), 'TrgInstGT')
    subplotimgV2(axs[1][1], prep_ins_for_vis(TrgInstPd.cpu(), mode='PD', dataset_name=dataset_name, debug=debug, blend_ratio=0.7, img=vis_trg_img), 'TrgInstPd')
